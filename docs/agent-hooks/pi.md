...
```bash
kouen-cli install-hooks pi
```

Writes `~/.pi/agent/extensions/kouen.ts`:
...
export function activate(api: any) {
  const notify = (body: string) =>
    execSync(
      `PATH="$HOME/Library/Application Support/Kouen/bin:$PATH" kouen-cli notify --surface "${process.env.HARNESS_SURFACE ?? ""}" --title "Pi" --body "${body}"`,
      { stdio: "ignore" }
    )
...
> reference and adjust the event names — the `kouen-cli notify` command itself
> is unchanged.
