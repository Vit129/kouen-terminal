...
- **Nerd Font / Powerline glyphs never tofu:** the app bundles **Symbols Nerd Font Mono** (MIT,
  `Apps/Kouen/Resources/Fonts/`) auto-activated via Info.plist `ATSApplicationFontsPath` — NOT a
...
**Agent Notch HUD:** `NotchPanelController` + `AgentNotchViewModel` + `AgentNotchRootView` (`UI/Notch/`) show at-a-glance agent rows on Macs with a notch; data comes from `AgentNotchProjection`. `Cmd+Shift+I` toggles the panel, waiting rows keep the notification body as the subtitle, and row titles include agent + source tab/session so the user can see where the alert came from. Click a row → `SessionCoordinator` focuses that session/tab.
...
```bash
kouen-cli ping && kouen-cli new-tab --workspace Default --cwd "$HOME"
# cd in GUI → sidebar/tab show folder name ~1s
kouen-cli notify --surface "$(kouen-cli list-surfaces | head -1)" --body test
```
