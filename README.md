...
- **AI browser control via MCP** — `harness-mcp` exposes tools like `harnessBrowserOpen` and `harnessBrowserSnapshot` (full list under [AI Browser Control](#ai-browser-control-harness-mcp)) so agents can drive the embedded browser pane directly. Upstream has no browser pane and no MCP server.
