...
/// a `Command` tree. Used by the `:` command prompt, `kouen-cli run`,
...
public enum CommandParser {
    ...
    /// Kouen verb, so `neww`, `splitw`, `killp`, `selectp`, `resizep`, … all work.
    ...
    /// in `bind-key`, the `:` prompt, a hook, or `display-popup -e`). Backs `kouen-cli
    ...
    private static func buildCommand(name rawName: String, tokens: [String]) throws -> Command {
        ...
        case "split-window":
            // Convention here mirrors the rest of Kouen: `.vertical` means
        ...
        case "set-window-option":
            // tmux `setw` — a window option; Kouen windows are tabs.
        ...
        case "find-window":
            ...
            // tmux defaults to matching everything (-CNT); Kouen defaults to the cheap
    ...
    /// flags (`-w` workspace, `-t` tab — Kouen's window); the translator resolves a
    ...
    /// tmux key-table name aliases. Kouen's vi-flavored copy-mode table is named
