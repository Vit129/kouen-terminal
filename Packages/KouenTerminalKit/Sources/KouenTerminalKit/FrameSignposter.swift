...
/// Points of Interest). **Off by default**: when `KOUEN_FRAME_SIGNPOSTS != 1` every call is a
...
///
/// Subsystem `com.vit129.kouen`, category `frame`. Enable with `PREVIEW_SIGNPOSTS=1 make preview`
/// (which launches `open … --args -KOUEN_FRAME_SIGNPOSTS 1`), by setting
/// `KOUEN_FRAME_SIGNPOSTS=1` in the app's launch environment (direct binary launch /
/// `xctrace record --template 'os_signpost' --launch …`), then read with
/// `log stream --predicate 'subsystem == "com.vit129.kouen"'`.
final class FrameSignposter: @unchecked Sendable {
    ...
    init() {
        ...
        // domain — `open -n Kouen.app --args -KOUEN_FRAME_SIGNPOSTS 1` lands in
        // `UserDefaults.standard` via NSArgumentDomain. One read at init; the hot path still
        // branches on the stored `enabled`.
        enabled = ProcessInfo.processInfo.environment["KOUEN_FRAME_SIGNPOSTS"] == "1"
            || UserDefaults.standard.bool(forKey: "KOUEN_FRAME_SIGNPOSTS")
        #if canImport(os)
        signposter = OSSignposter(subsystem: "com.vit129.kouen", category: "frame")
        logger = Logger(subsystem: "com.vit129.kouen", category: "frame")
        #endif
    }
    ...
    /// with `log stream --predicate 'subsystem == "com.vit129.kouen"'` (no Instruments needed).
