...
public enum ThemeManager {
    public static let defaultThemeName = KouenThemeCatalog.defaultThemeName
    ...
    /// Kouen chrome only, so tools such as Claude Code keep their native colors.
    ...
    public static let featuredThemes = [
        KouenThemeCatalog.defaultThemeName,
        "Catppuccin Mocha",
        "Dracula",
        "Tokyo Night",
        "Nord",
    ...
    /// theme seeds these into `KouenSettings`, after which the user may edit
    ...
    public static func allThemeNames() -> [String] {
        [defaultDisplayName]
            + featuredThemes
            + KouenThemeCatalog.allThemes.map(\.name)
                .filter { !featuredThemes.contains($0) && $0 != defaultDisplayName }
    }

    private static func themed(_ name: String) -> KouenThemeDefinition? {
        let resolved = (name == defaultDisplayName) ? defaultThemeName : name
        return KouenThemeCatalog.theme(named: resolved) ?? KouenThemeCatalog.theme(named: defaultThemeName)
    }
}
