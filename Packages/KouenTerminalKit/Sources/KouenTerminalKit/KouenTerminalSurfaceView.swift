...
// `KouenTheme.RGBColor` in this file. Pin the name to ours.
typealias RGBColor = KouenTheme.RGBColor
...
@MainActor
public final class KouenTerminalSurfaceView: NSView {
    ...
    public init(
        ...
        let theme = KouenThemeCatalog.theme(named: themeName)
            ?? KouenThemeCatalog.theme(named: ThemeManager.defaultThemeName)!
    ...
    // Full renderer stats for the frame-pacing kouen (encodedRows/reusedRows/uploadBytes/...).
    ...
    func testingSetSelectionColors(
        background: KouenTheme.RGBColor?, foreground: KouenTheme.RGBColor?
    ) {
    ...
    /// (`CSI > c`). Resolved by the host from the `terminal-identity` option (KouenCore
    ...
    private static var retiredSurfaces: [KouenTerminalSurfaceView] = []
