...
public final class TerminalHostView: NSView {
    ...
    private let nativeView: KouenTerminalSurfaceView
    public var surfaceView: KouenTerminalSurfaceView { nativeView }
    ...
    private var cachedSettings: KouenSettings?
    ...
    public init(
        ...
        kouenSurfaceEnv: String? = nil,
        settings: KouenSettings? = nil,
    ...
    private func configureNative(_ native: KouenTerminalSurfaceView, io: SurfaceIO, inputGate: InputGate) {
        wantsLayer = true
        ...
        native.onDesktopNotification = { [weak self] title, body in
            ...
            self.hostDelegate?.terminalHostDidRequestDesktopNotification(
                title: title ?? "Kouen", body: body, surfaceID: self.surfaceID)
        }
    ...
    private func applyNativeAppearance() {
        ...
        nativeView.configureAppearance(
            ...
            canvasOpacity: KouenSettings.clampedOpacity(settings.backgroundOpacity),
            ...
            minimumContrast: KouenSettings.clampedContrast(settings.minimumContrast),
    ...
    private func nativeOutputPaletteHex(settings: KouenSettings) -> [String?] {
    ...
    public func applySettings(_ settings: KouenSettings) {
    ...
    /// this from the `terminal-identity` option (KouenCore `TerminalIdentity`).
    ...
    @discardableResult
    private func ensureDaemonSurface(cwd: String?, shell: String, settings: KouenSettings?) -> Bool {
        do {
            ...
            fputs("Kouen: ensureSurface failed for \(surfaceID.uuidString): \(error)\n", kouenStderr)
        }
        return false
    }
    ...
    private func startDaemonOutput(resetBeforeReplay: Bool = false) {
        ...
        do {
            ...
            fputs("Kouen: output subscription failed for \(surfaceID.uuidString): \(error)\n", kouenStderr)
        }
    }
    ...
    private func scheduleDaemonReconnect() {
        ...
        reconnectQueue.asyncAfter(deadline: .now() + delay) {
            ...
            let subscription = try? client.attachReplayingSurfaceOutput(
                surfaceID: sid, label: "Kouen.app", onReplay: onReplay, onData: onData, onEnd: onEnd
            )
