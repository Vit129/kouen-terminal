...
///   (no rc edit needed).
/// - **zsh/bash**: the script is written under the Kouen home and a guarded, backed-up,
...
public enum ShellCompletionInstaller {
    ...
    // Unchanged by the Kouen->Kouen rename: this sentinel is already written into users' real
    ...
    private static let markerBegin = "# >>> Kouen CLI completions >>>"
    private static let markerEnd = "# <<< Kouen CLI completions <<<"
    ...
    /// under the Kouen home's `completions/` directory (sourced from the rc). `homeOverride` (the
    ...
    public static func scriptURL(for shell: ShellIntegration.Shell, homeOverride: URL? = nil) -> URL {
        ...
        case .fish:
            ...
            return KouenPaths.fishCompletionURL
        case .zsh, .bash:
            let appSupport = homeOverride.map {
                $0.appendingPathComponent("Library/Application Support/Kouen", isDirectory: true)
    ...
    /// script under the Kouen home and wire a guarded `source` block into the rc (idempotent,
