...
/// Linux systemd `--user` backend: installs `~/.config/systemd/user/kouen-daemon.service` and
...
public struct SystemdUserInstaller: ServiceInstaller {
    public static let serviceName = "kouen-daemon.service"
    public init() {}
    ...
    /// restarted on failure, logging to the daemon log. `KOUEN_HOME` is pinned so the service and
    /// interactive `kouen-cli` resolve the same socket/sessions.
    public static func unitContents(daemonPath: URL, kouenHome: URL, logPath: URL) -> String {
        """
        [Unit]
        ...
        Environment=KOUEN_HOME=\(kouenHome.path)
        Restart=on-failure
        RestartSec=2
    ...
    public func install(daemonPath: URL, kouenHome: URL = KouenPaths.applicationSupport) throws -> ServiceInstallReport {
        ...
        let desired = Self.unitContents(daemonPath: daemonPath, kouenHome: kouenHome, logPath: KouenPaths.daemonLogURL)
        ...
        if !activated {
            ...
            fputs("kouen: `systemctl --user enable --now \(Self.serviceName)` failed: \(result.output)\n", kouenStderr)
            fputs("kouen: if this is a headless host, run `loginctl enable-linger $USER` and retry.\n", kouenStderr)
        }
    ...
    private static func runSystemctl(_ arguments: [String]) -> (status: Int32, output: String) {
        ...
        let outputURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("kouen-systemctl-\(UUID().uuidString).log")
