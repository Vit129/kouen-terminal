...
public enum DaemonSessionError: Error, CustomStringConvertible {
    ...
    public var description: String {
        ...
        case .unexpectedResponse: "Unexpected response from KouenDaemon"
        }
    }
}

/// Client wrapper used by Kouen.app for all session mutations. Can be repointed at a different
...
public final class DaemonSessionService: @unchecked Sendable {
    ...
    /// the UI with no signal. This logs a `slow IPC` line (subsystem `com.vit129.kouen`, category
    ...
    /// `log stream --predicate 'subsystem == "com.vit129.kouen" && category == "ipc"'`. No behavior
    ...
    private final class LatencyMonitor: @unchecked Sendable {
        ...
        #if canImport(os)
        private let logger = Logger(subsystem: "com.vit129.kouen", category: "ipc")
        private let signposter = OSSignposter(subsystem: "com.vit129.kouen", category: "ipc")
        #endif
