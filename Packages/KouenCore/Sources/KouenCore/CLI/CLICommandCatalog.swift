...
/// One `kouen-cli` subcommand: its canonical name, a one-line summary, any aliases, and whether
...
/// The canonical catalog of `kouen-cli` subcommands — the single source of truth for shell
/// completions (`CompletionGenerator`, used by `kouen-cli completions` and the installer) so the
/// command list never drifts across the fish/zsh/bash scripts. Keep in sync with the dispatch
/// `switch` in `KouenCLI.main` (a test asserts the documented core commands are present).
public enum CLICommandCatalog {
    public static let commands: [CLICommand] = [
        ...
        .init("daemon", "Run the daemon in the foreground (execs KouenDaemon)"),
    ]
