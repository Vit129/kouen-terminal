...
/// config file are preserved — only the `kouen` key under `mcpServers` is touched.
public enum MCPConfigWriter {
    ...
    /// Returns true when `mcpServers.kouen` already exists in the agent's config file.
    ...
    /// Adds `mcpServers.kouen` pointing at `mcpBinaryPath`. Idempotent — re-adding
    ...
    /// Removes `mcpServers.kouen` from the agent's config file. No-op if not present.
