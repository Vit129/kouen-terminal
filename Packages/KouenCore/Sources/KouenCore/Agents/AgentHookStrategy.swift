...
/// How Kouen writes one agent's hook config. Each installable `AgentKind` maps to exactly
...
enum AgentHookStrategy {
    ...
    /// Claude Code, Codex. `managedEvents` are pruned of Kouen-owned entries before each merge
    ...
    /// Write a dedicated, Kouen-owned JSON file verbatim — Grok's `~/.grok/hooks/kouen.json`.
    /// We own the whole file (the agent merges every `*.json` in the dir), so install overwrites
    /// idempotently; there's nothing to prune or preserve. The pre-rename `kouen.json` sibling
    /// is cleaned up via `legacyHookFiles`, same as any other older-Kouen orphan.
    case ownJSONFile(filename: String, payload: [String: Any])

    /// Write a dedicated, Kouen-owned text file verbatim — a JS plugin (OpenCode) or TS
    ...
    /// Upsert a sentinel-delimited, Kouen-marked region into a text config that Kouen doesn't
