...
public final class EnvironmentStore: @unchecked Sendable {
    ...
    public init(url: URL? = nil) {
        self.url = url ?? KouenPaths.applicationSupport.appendingPathComponent("environment.json")
        ...
        if let decoded = try? JSONDecoder().decode(Persisted.self, from: data) {
            ...
            KouenPaths.backupCorruptFile(at: self.url, label: "KouenDaemon")
        }
    }
    ...
    private func save() {
        ...
        KouenPaths.atomicWrite(data, to: url, label: "KouenDaemon")
    }
}
