...
public final class OptionStore: @unchecked Sendable {
    ...
    public init(url: URL? = nil) {
        self.url = url ?? KouenPaths.applicationSupport.appendingPathComponent("options.json")
    ...
    /// tmux option-name aliases mapped onto the Kouen option they correspond to.
    /// `default-terminal` would set TERM in tmux; Kouen pins TERM and exposes the
    ...
    public static let builtinDefaults: [String: Value] = [
        ...
        // Lifecycle/timing. `remain-on-exit` on (Kouen's safe default; tmux defaults off)
        ...
        // `wrap-search`: when on (default, tmux default), search wraps around the buffer;
        // when off, searching stops at the top/bottom without wrapping.
        "wrap-search": .bool(true),
        // (`bind -r`) so the key repeats without re-pressing the prefix. Read by `PrefixKeymap`.
        "repeat-time": .int(500),
        // How Kouen identifies itself to programs (`TERM_PROGRAM` env + XTVERSION/secondary-DA
        // replies). `compatible` (default) reports a protocol-compatible identity so tools like
        // Claude Code enable Kitty-keyboard / Shift+Enter immediately; `kouen` reports the true
        ...
        "set-titles-string": .string("#{session_name}:#{window_name} — Kouen"),
        ...
        // tmux default is off; Kouen default is off to preserve Kouen's "sessions survive quit" promise.
    ...
    private func save() {
        ...
        KouenPaths.atomicWrite(data, to: url, label: "KouenDaemon") // temp + rename, logs on failure
    }
