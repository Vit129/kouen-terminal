...
/// the launch path. Enable with `KOUEN_STARTUP_METRICS=1`; the timeline is logged
...
public final class StartupMetrics: @unchecked Sendable {
    ...
    /// `~/Library/Application Support/Kouen/logs/startup.log` (or under `KOUEN_HOME`).
    public static var logURL: URL { KouenPaths.logsDirectory.appendingPathComponent("startup.log") }

    /// Reads `KOUEN_STARTUP_METRICS` once. Tests pass `enabled:` explicitly for
    ...
    public init(enabled: Bool? = nil) {
        if let enabled {
            ...
            let on = ProcessInfo.processInfo.environment["KOUEN_STARTUP_METRICS"] == "1"
            self.enabled = on
    ...
    public func report() -> String {
        guard enabled else { return "startup metrics: disabled (set KOUEN_STARTUP_METRICS=1)" }
        lock.lock()
        let snapshot = marks
        lock.unlock()
