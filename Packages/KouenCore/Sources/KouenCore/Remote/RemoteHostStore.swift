...
/// A remote machine running `KouenDaemon`, reachable by forwarding its control socket over SSH.
public struct RemoteHost: Codable, Sendable, Equatable, Identifiable {
    ...
    /// Path to `kouen.sock` on the remote machine.
...
public final class RemoteHostStore: @unchecked Sendable {
    ...
    /// class the audit flagged for `kouen-cli remote add`.
    ...
    /// acquisition *and* an inter-process file lock (`flock`) so concurrent `kouen-cli remote add`
    ...
    public func upsert(_ host: RemoteHost) -> MutationResult {
        lock.lock()
        defer { lock.unlock() }
        return KouenPaths.withFileLock(KouenPaths.remoteHostsLockURL) {
    ...
    private func loadLocked() -> [RemoteHost] {
        let url = KouenPaths.remoteHostsURL
        ...
        KouenPaths.backupCorruptFile(at: url, label: "RemoteHostStore")
        return []
    }
    ...
    private func saveLocked(_ hosts: [RemoteHost]) -> Bool {
        ...
        return KouenPaths.atomicWrite(data, to: KouenPaths.remoteHostsURL, label: "RemoteHostStore")
    }
