...
/// stream) can drive a remote Kouen daemon unchanged. This remote transport
...
public final class SSHTunnelManager: @unchecked Sendable {
    ...
    public func endpoint(for host: RemoteHost, waitTimeout: TimeInterval = 10) throws -> Endpoint {
        let localSocket = KouenPaths.tunnelSocketURL(forHost: host.name)
    ...
    private func spawnTunnel(host: RemoteHost, localSocket: URL) throws {
        try? FileManager.default.createDirectory(
            at: KouenPaths.tunnelsDirectory, withIntermediateDirectories: true,
        ...
        // every kouen-cli invocation that opened a tunnel leaves an orphaned ssh
    ...
    /// Runs `kouen-cli socket-path` on the remote host over ssh and returns the path it prints —
    /// a one-shot exec (no `-N -L` forward) used to auto-fill a `RemoteHost`'s socket path instead of
    /// asking the user to run `kouen-cli doctor` on the remote by hand. Blocking — call off the
    ...
    public static func detectSocketPath(
        ...
        args += [try validatedSSHTarget(sshTarget), "kouen-cli", "socket-path"]
        ...
        guard process.terminationStatus == 0 else {
            throw SSHTunnelError.launchFailed(
                "ssh exited with status \(process.terminationStatus) while detecting the remote socket "
                    + "path — check the host is reachable and `kouen-cli` is on its PATH")
        }
