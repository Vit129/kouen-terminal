...
public struct ImportedTerminalConfig: Sendable, Equatable {
    ...
    /// Ghostty dual-appearance `theme = light:Name,dark:Name` — mapped onto Kouen's
    ...
    /// WCAG minimum-contrast ratio (1…21). Applied to `KouenSettings.minimumContrast` and
    ...
    public var signature: String {
        ...
        parts.append(KouenSettings.normalizedPalette(paletteHex).map { $0 ?? "" }.joined(separator: ","))
...
/// Reads an existing terminal config from disk and pulls values that map cleanly to Kouen —
...
public enum TerminalConfigImporter {
    ...
    /// files for duplicated keys, matching how Kouen has historically treated
    ...
    private static func normalizeHex(_ raw: String) -> String? {
        ...
        return KouenSettings.normalizedHex(trimmed)
    }
...
private extension ImportedTerminalConfig {
    ...
    private func mergePalette(_ newer: [String?], over older: [String?]) -> [String?] {
        let normalizedNewer = KouenSettings.normalizedPalette(newer)
        let normalizedOlder = KouenSettings.normalizedPalette(older)
