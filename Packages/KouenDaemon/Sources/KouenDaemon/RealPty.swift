...
public final class RealPty: @unchecked Sendable {
    ...
    /// running app + any number of `kouen-cli attach` clients).
    ...
    /// (Kouen-owned `$KOUEN`/`$KOUEN_SURFACE` plus user `set-environment`).
    ...
    /// daemon advertises so capability-detecting tools (Claude Code) recognize Kouen and enable
    ...
    public init(
        ...
        // `KOUEN` (the $TMUX analog the OSC 133 scripts gate on) + session `set-environment`
    ...
    /// any `kouen-cli attach` simply see fresh output begin.
    public func respawn(clearHistory: Bool, fallbackCwd: String? = nil) {
        ...
        if oldFD >= 0, kouen_pty_get_winsize(oldFD, &probedRows, &probedCols) == 0 {
            oldRows = probedRows
            oldCols = probedCols
        } else {
            oldRows = 24
        ...
        do {
            ...
            fputs("KouenDaemon: respawn failed for \(id): \(error)\n", kouenStderr)
    ...
    private static func spawnOnPTY(
        ...
        let master = kouen_open_pty_master(&slaveBuf, slaveBuf.count)
        ...
        if pid == 0 {
            ...
            let slave = slavePath.map { kouen_open_rdwr($0) } ?? -1
            ...
            _ = kouen_pty_make_controlling(slave)
            _ = kouen_pty_set_winsize(slave, rows, cols)
            _ = dup2(slave, 0)
            _ = dup2(slave, 1)
            _ = dup2(slave, 2)
    ...
    public func resize(rows: UInt16, cols: UInt16) {
        ...
        guard dupFd >= 0 else { return }
        defer { sysClose(dupFd) }
        _ = kouen_pty_set_winsize(dupFd, rows, cols)
    }
    ...
    public func currentSize() -> (rows: Int, cols: Int)? {
        ...
        var rows: UInt16 = 0
        var cols: UInt16 = 0
        guard kouen_pty_get_winsize(fd, &rows, &cols) == 0, rows > 0, cols > 0 else { return nil }
    ...
    private func currentWinsize() -> (cols: Int, rows: Int) {
        ...
        var probedRows: UInt16 = 0
        var probedCols: UInt16 = 0
        if fd >= 0, kouen_pty_get_winsize(fd, &probedRows, &probedCols) == 0, probedCols > 0, probedRows > 0 {
    ...
    public func captureGrid(start: Int?, end: Int?, joinWrapped: Bool) -> String {
        ...
        guard let term = KouenGridTerminal(cols: size.cols, rows: size.rows) else { return "" }
    ...
    /// P34 F3 (`kouenGetLastBlock`/`kouenGetBlock`): a command block reconstructed the same
    ...
    public func block(id: Int?) -> BlockSummary? {
        let size = currentWinsize()
        guard let term = KouenGridTerminal(cols: size.cols, rows: size.rows) else { return nil }
        term.maxScrollbackLines = 100_000
        term.feed(scrollbackData(includeHistory: true))
