...
/// Pure, side-effect-free lifecycle decisions shared by `KouenDaemonMain`. Kept in the
...
public enum DaemonLifecycle {
    ...
    public enum PriorInstanceDecision: Equatable {
        /// The recorded PID is a live KouenDaemon — refuse to start (two daemons sharing a
    ...
    /// a KouenDaemon binary. The authoritative socket ping in `DaemonServer.start()` remains
    ...
    public static func priorInstanceDecision(
        ...
        guard let path = executablePath(priorPID),
              URL(fileURLWithPath: path).lastPathComponent.contains("KouenDaemon")
        else {
