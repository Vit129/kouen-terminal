...
struct VersionBannerStore {
    ...
    init(url: URL = KouenPaths.versionStateURL) {
        self.url = url
    }
    ...
    @discardableResult
    func markSeen(build: Int = KouenVersion.build, version: String = KouenVersion.short) -> Bool {
        guard let data = try? JSONEncoder().encode(State(lastSeenBuild: build, lastSeenVersion: version)) else {
            return false
        }
        return KouenPaths.atomicWrite(data, to: url, label: "version-state")
    }
