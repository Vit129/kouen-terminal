...
public final class DaemonServer: @unchecked Sendable {
    ...
    /// The snapshot subscriber fd for the main GUI app — label "KouenGUI".
    /// Falls back to any subscriber when only one exists (e.g. tests).
    private var guiBrowserFD: Int32? {
        snapshotSubscribers.first(where: { clients[$0]?.label == "KouenGUI" })
    ...
    public func start() throws {
        ...
        if FileManager.default.fileExists(atPath: KouenPaths.socketURL.path) {
            if case .pong = try? DaemonClient().request(.ping, timeout: 0.2) {
                throw DaemonError.alreadyRunning
            }
            try FileManager.default.removeItem(at: KouenPaths.socketURL)
        }

        // Validate the socket path fits `sun_path` before binding, so a deep KOUEN_HOME fails
        // with a clear message instead of `strncpy`-truncating and binding the wrong socket.
        let socketPath = try KouenPaths.validatedSocketPath()
        ...
        if chmod(KouenPaths.socketURL.path, 0o600) != 0 {
            close(fd)
            throw DaemonError.bindFailed
        }
        // `SOMAXCONN`, not a small fixed backlog: the daemon serves the GUI plus any number of
        // `kouen-cli` clients, and a burst of near-simultaneous connects (e.g. several attach
        ...
        source.resume()
        listener = source
        fputs("KouenDaemon listening at \(KouenPaths.socketURL.path)\n", kouenStderr)
    }
    ...
    private func acceptConnection(listenerFD: Int32) {
        ...
        let peer = kouen_peer_uid(clientFD)
        ...
        _ = kouen_set_nonblocking(clientFD)
    ...
    private func handleSubscribe(surfaceID: String, label: String?, fd: Int32) {
        ...
        // (Kouen.app, kouen-cli attach, etc.). Register it so `list-clients`
        ...
        await withCheckedContinuation { continuation in
            queue.async {
                guard let guiFD = self.guiBrowserFD else {
                    continuation.resume(returning: .error("Kouen GUI is not running or connected"))
                    return
                }
...
public enum DaemonError: Error, CustomStringConvertible {
    ...
    public var description: String {
        switch self {
        case .alreadyRunning: "KouenDaemon is already running"
