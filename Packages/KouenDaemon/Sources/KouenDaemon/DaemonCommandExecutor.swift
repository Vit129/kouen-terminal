        ...
        default:
            ...
            case .clientLocal:
                ...
                if case let .findWindow(pattern, name, content, title, scopeTarget) = command, content {
                    ...
                    fputs("KouenDaemon: hook command has no server-side action: \(command.shortDescription)\n", kouenStderr)
                }
            case .unresolved:
                fputs("KouenDaemon: hook command had no resolvable target: \(command.shortDescription)\n", kouenStderr)
            }
        }
    }
}
