...
/// `REVIEW-graphify-harness-2026-07-03.md` Part 3 for the original crash report).
///
...
///
/// Lives in HarnessCore (not HarnessApp) because both HarnessApp's `DesktopNotifier` and
/// HarnessOnboarding's `NotificationPermission` need it, and HarnessOnboarding cannot depend on
/// the app target — HarnessCore is the shared package both already depend on.
