...
/// Errors raised when reading or validating a `.kouentheme` document.
...
/// The shareable `.kouentheme` file format — a versioned, human-readable JSON document
/// describing a full Kouen theme: canvas colors, ANSI palette, and the appearance knobs
...
/// `KouenThemeDefinition` (which is colors-only): a document can seed both the catalog
...
public struct ThemeDocument: Codable, Equatable, Sendable {
    ...
    public static let fileExtension = "kouentheme"
    ...
    public init(
        definition: KouenThemeDefinition,
        appearance: Appearance? = nil,
        author: String? = nil
    ) {
        self.init(
    ...
    public var themeDefinition: KouenThemeDefinition {
        KouenThemeDefinition(
            name: name,
    ...
    /// Decode and validate a document from `.kouentheme` bytes.
