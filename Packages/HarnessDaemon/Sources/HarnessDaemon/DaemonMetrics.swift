...
/// launching the daemon with `HARNESS_DAEMON_METRICS=1`; the snapshot is then
...
public final class DaemonMetrics: @unchecked Sendable {
    ...
    /// Default initializer reads the `HARNESS_DAEMON_METRICS` environment variable
    /// once. Tests pass `enabled:` explicitly for deterministic behavior.
    public init(enabled: Bool? = nil) {
        if let enabled {
            self.enabled = enabled
        } else {
            self.enabled = ProcessInfo.processInfo.environment["HARNESS_DAEMON_METRICS"] == "1"
        }
    }
    ...
    public func summary() -> String {
        guard enabled else { return "metrics: disabled (set HARNESS_DAEMON_METRICS=1)" }
        let s = snapshot()
        return String(
