...
/// Snapshot of daemon health used by `kouen-cli daemon-stats` and support tooling.
public struct DaemonStats: Codable, Sendable {
    ...
    /// Marketing version (`KouenVersion.short`) of the running daemon. Optional because
    ...
    /// Build number (`KouenVersion.build`) of the running daemon — the handshake the app
...
public extension DaemonStats {
    ...
    /// (the caller's `KouenVersion.build`). A nil build is a daemon too old to know
...
/// Summary of a connected client (a Kouen.app instance or an attached
/// `kouen-cli` process). Used by `list-clients` / `detach-client`.
