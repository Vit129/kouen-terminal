...
/// The one-click installer logic for the standalone Kouen CLI onboarding experience.
///
/// It is deliberately self-contained (no link to KouenCore) but follows the exact
/// same paths, plist template, and launchctl patterns as the real `kouen-cli install`
/// and `LaunchAgentInstaller` so that the result is 100% compatible with a future
/// full Kouen.app or CLI-only distribution.
@MainActor
enum BinaryInstaller {
    enum DetectionStatus: Equatable {
        ...
        var display: String {
            switch self {
            // Fall back to the detected binary's name so the Daemon row reads "Found KouenDaemon"
            // — not the CLI's "Found kouen-cli" (both rows pass version: nil today).
    ...
    enum InstallError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .missingBundledTools(let messages):
                return (messages + ["Kouen.app is missing its bundled command-line tools. Rebuild or reinstall Kouen."])
                    .joined(separator: "\n")
            }
        }
    }

    // MARK: - Detection (the locations the real kouen-cli install also checks)

    /// The `Contents/MacOS` directory of the host app. Embedded in Kouen.app this is where
    /// the bundled `kouen-cli` + `KouenDaemon` live (copied in by the "Copy Bundled Tools"
    ...
    static func detectCLI() -> DetectionStatus {
        let candidates: [URL] = [
            // 1. Inside the running app bundle's MacOS dir (Kouen.app embeds the binaries)
            bundledMacOSDir?.appendingPathComponent("kouen-cli"),
            // 2. Next to the app (the "all-in-one DMG" layout)
            Bundle.main.bundleURL.deletingLastPathComponent().appendingPathComponent("kouen-cli"),
            // 3. Inside a sibling Kouen.app (if the user has the GUI version)
            URL(fileURLWithPath: "/Applications/Kouen.app/Contents/MacOS/kouen-cli"),
            // 4. Already installed by a previous run or the GUI app
            KouenCLIPaths.installedCLIPath,
            // 5. Dev builds
            URL(fileURLWithPath: ".build/release/kouen-cli", relativeTo: URL(fileURLWithPath: FileManager.default.currentDirectoryPath))
        ].compactMap { $0 }
    ...
    static func detectDaemon() -> DetectionStatus {
        let candidates: [URL] = [
            bundledMacOSDir?.appendingPathComponent("KouenDaemon"),
            Bundle.main.bundleURL.deletingLastPathComponent().appendingPathComponent("KouenDaemon"),
            URL(fileURLWithPath: "/Applications/Kouen.app/Contents/MacOS/KouenDaemon"),
            KouenCLIPaths.installedDaemonPath,
            URL(fileURLWithPath: ".build/release/KouenDaemon", relativeTo: URL(fileURLWithPath: FileManager.default.currentDirectoryPath))
        ].compactMap { $0 }
        for url in candidates {
            if FileManager.default.fileExists(atPath: url.path) {
                return .found(version: nil, path: url)
    ...
    static func performInstall(cliSource: URL?, daemonSource: URL?) throws -> InstallReport {
        ...
        try KouenCLIPaths.ensureDirectories()

        // Re-running onboarding from an *older* Kouen.app (Help → Welcome re-opens this wizard)
        // must never silently downgrade a newer installed daemon/CLI. The bundled CLI + daemon
        // always ship from the same app build, so a single source-vs-installed `kouen-cli`
        ...
        let cliSrc = cliSource ?? findBestSource(named: "kouen-cli")
        let daemonSrc = daemonSource ?? findBestSource(named: "KouenDaemon")
        let sourceBuild = cliSrc.flatMap { buildNumberProbe($0) }
        let installedBuild = buildNumberProbe(KouenCLIPaths.installedCLIPath)

        // Copy CLI
        if let src = cliSrc {
            let dest = KouenCLIPaths.installedCLIPath
            let outcome = try copyReplacing(src: src, dest: dest, executable: true,
                                            sourceBuild: sourceBuild, installedBuild: installedBuild)
            messages.append(outcome.message(binary: "kouen-cli", dest: dest))
            cliOK = true
        } else {
            messages.append("kouen-cli source not found; skipping binary copy")
        }

        // Copy Daemon — same build comparison as the CLI (they ship together from one app build).
        if let src = daemonSrc {
            let dest = KouenCLIPaths.installedDaemonPath
            let outcome = try copyReplacing(src: src, dest: dest, executable: true,
                                            sourceBuild: sourceBuild, installedBuild: installedBuild)
            messages.append(outcome.message(binary: "KouenDaemon", dest: dest))
            daemonOK = true
        } else {
            messages.append("KouenDaemon source not found; skipping binary copy")
        }
        ...
        if daemonOK || FileManager.default.fileExists(atPath: KouenCLIPaths.installedDaemonPath.path) {
            let daemonPath = KouenCLIPaths.installedDaemonPath
            let home = KouenCLIPaths.applicationSupport
            let log = home.appendingPathComponent("logs/daemon.log")
            try? FileManager.default.createDirectory(at: log.deletingLastPathComponent(), withIntermediateDirectories: true)

            let plistContent = launchAgentPlist(daemonPath: daemonPath, kouenHome: home, logPath: log)
            let plistURL = KouenCLIPaths.launchAgentURL
            ...
            _ = runLaunchctl(["enable", "gui/\(getuid())/\(KouenCLIPaths.launchAgentLabel)"])
        } else {
    ...
    private static func findBestSource(named binary: String) -> URL? {
        ...
        let app = URL(fileURLWithPath: "/Applications/Kouen.app/Contents/MacOS/\(binary)")
    ...
    /// Returns nil when the binary is absent or doesn't answer (e.g. KouenDaemon has no version
    ...
    private static func launchAgentPlist(daemonPath: URL, kouenHome: URL, logPath: URL) -> String {
        """
        ...
        <dict>
            <key>Label</key>
            <string>\(KouenCLIPaths.launchAgentLabel)</string>
