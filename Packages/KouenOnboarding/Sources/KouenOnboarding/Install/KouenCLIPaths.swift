...
/// Mirrors the relevant pieces of KouenCore.KouenPaths so the wizard can
/// install to exactly the same locations the real kouen-cli expects — without
/// any build or runtime dependency on the main monorepo.
enum KouenCLIPaths {
    static var applicationSupport: URL {
        // Allow KOUEN_HOME override exactly like the real daemon/CLI (useful for preview).
    ...
    static var launchAgentURL: URL {
        FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("Library/LaunchAgents/com.vit129.kouen.daemon.plist")
    }

    static let launchAgentLabel = "com.vit129.kouen.daemon"
