...
struct SetupStepView: View {
    ...
    /// Detected agents that don't yet have Kouen hooks — the ones we offer to wire up.
    ...
    private var hooksDetail: String {
        ...
        return "Notify Kouen when \(names) finish or need input"
    }
    ...
    var body: some View {
        VStack(spacing: 24) {
            StepIntro(
                eyebrow: "Install",
                title: isSuccess ? "Kouen is installed." : "Install the local tools.",
                bodyText: isSuccess
                    ? "The CLI and daemon are in place. The LaunchAgent is registered for your user account."
                    : "Kouen copies the CLI and daemon into your Library, then sets up the background process."
            )

            VStack(spacing: 0) {
                statusRow(title: "CLI", detail: "kouen-cli", status: cliStatus)
                    .padding(.vertical, 13)
                Rectangle().fill(.white.opacity(0.075)).frame(height: 1)
                statusRow(title: "Daemon", detail: "KouenDaemon", status: daemonStatus)
                    .padding(.vertical, 13)
                Rectangle().fill(.white.opacity(0.075)).frame(height: 1)
    ...
    /// Install Kouen hooks for every detected agent that doesn't have them yet, then refresh.
    ...
    private func performInstall() {
        ...
        Task {
            do {
                ...
                errorMessage = "Install failed: \(error.localizedDescription)\nRun `kouen-cli install` from Kouen.app or a build output."
            }
            isInstalling = false
        }
    }
