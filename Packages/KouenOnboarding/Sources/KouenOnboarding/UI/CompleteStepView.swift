    ...
    var body: some View {
        VStack(spacing: 26) {
            StepIntro(
                eyebrow: "Ready",
                title: "You are set.",
                bodyText: "Kouen is ready. The CLI is installed, your shell can find it, and the daemon is running in the background."
            )

            VStack(spacing: 12) {
                CommandRow("kouen-cli ping", "Check the daemon")
                CommandRow("kouen-cli list-surfaces", "List your sessions")
                CommandRow("kouen-cli attach-window --tab <id>", "Attach to a tab")
            }
            .frame(maxWidth: 420)
