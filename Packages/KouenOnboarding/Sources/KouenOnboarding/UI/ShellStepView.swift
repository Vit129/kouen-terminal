...
struct ShellStepView: View {
    ...
    /// No shell has the Kouen PATH block yet — leaving now means `kouen-cli` isn't on PATH.
    ...
    var body: some View {
        VStack(spacing: 24) {
            StepIntro(
                eyebrow: "Shell",
                title: allConfigured || success ? "Your PATH is ready." : "Add Kouen to your shell.",
                bodyText: "Kouen updates zsh, bash, and fish with timestamped backups. Open a new terminal after this."
            )
            ...
            // Non-blocking heads-up if the user leaves this step with no profile updated: kouen-cli
