...
/// Injection seam that lets the (deliberately KouenCore-free) onboarding module offer
/// agent-hook setup without importing the core installer. `KouenApp` populates these
...
public enum OnboardingEnvironment {
    ...
    /// Install Kouen notification hooks for the agent `id`; returns true on success.
    public static var installHooks: (_ agentID: String) -> Bool = { _ in false }

    /// The canonical fish completion script for `kouen-cli`, generated from `CLICommandCatalog`
    /// (`CompletionGenerator.script(for: .fish)`). `KouenApp` populates this; left unset (preview/
