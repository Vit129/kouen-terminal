...
//   Packages/KouenTerminalKit/Sources/KouenTerminalKit/GridCompositor.swift
// This INTENTIONAL simplified copy avoids pulling KouenTerminalEngine, KouenCopyMode,
// and KouenCore into the onboarding package — the first-run wizard only needs display
// rendering (no PTY, no damage tracking, no real grid access). See the note in the
// authoritative file for the full rationale.
//
// Ported (near-verbatim) from Packages/KouenTerminalKit/.../GridCompositor.swift.
// Three edits versus the monorepo original:
//   1. drop the KouenCopyMode/KouenCore/KouenTerminalEngine imports (types are inlined here),
...
/// box-drawing borders + status rows) — the core of the `kouen attach` renderer. Pure
/// and deterministic.
