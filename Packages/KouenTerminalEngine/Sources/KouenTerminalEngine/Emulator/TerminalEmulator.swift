...
/// `kouen attach` compositor both build on it; `KouenGridTerminal` in
/// KouenTerminalKit is a thin wrapper that adapts it to the existing call sites.
///
...
public final class TerminalEmulator: VTParserHandler {
    ...
    // `terminal-identity` option (KouenCore `TerminalIdentity`) — the engine is dependency-free,
    ...
    public var terminalName: String = "Kouen"
    ...
    /// The most recently *finished* block, if any — what `kouenGetLastBlock` (F3) reads.
    ...
    /// A non-`?` payload is a *set*, which Kouen ignores — the theme owns the canvas colors.
