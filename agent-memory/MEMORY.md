...
## Active Decisions
- [2026-06-29] **Post-migrate CPU = SwiftUI `.repeatForever` re-rendering a whole NSHostingView ViewGraph every frame; memory is clean.** Live profile of running 3.11.7/183: RSS ~110 MB stable (no leak — all leak fixes confirmed live), but ~42% CPU with `sample` showing 33% in `NSHostingView.layout()` → `ViewGraph.updateOutputs(at:)`. Primary suspect `TerminalTabBarView.swift:469` `workingDot` `.repeatForever` (pulses while any tab is `working`). Same class as the fixed Notch CPU bug. Fix = CABasicAnimation on a CALayer (off SwiftUI) or isolate the dot in its own host. Audit `repeatForever|TimelineView|Timer.publish` in SwiftUI views before blaming snapshot/data. NOT yet fixed. See `knowledge/bugs/notch-cpu-animation.md` Instance 2.
