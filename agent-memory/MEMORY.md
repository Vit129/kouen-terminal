...
## Active Decisions
- [2026-06-29] **Post-migrate CPU = SwiftUI `.repeatForever` re-rendering a whole NSHostingView ViewGraph every frame; memory is clean.** Live profile of running 3.11.7/183: RSS ~110 MB stable (no leak — all leak fixes confirmed live), but ~42% CPU with `sample` showing 33% in `NSHostingView.layout()` → `ViewGraph.updateOutputs(at:)`. Primary suspect `TerminalTabBarView.swift:469` `workingDot` `.repeatForever` (pulses while any tab is `working`). Same class as the fixed Notch CPU bug. Audit `repeatForever|TimelineView|Timer.publish` in SwiftUI views before blaming snapshot/data. **FIXED `dd7a78c`** — both `workingDot` and notch `NotchStatusDot` pulses moved to CALayer (`CABasicAnimation`/`CAAnimationGroup`); SwiftUI rendering untouched, animation off the ViewGraph. Build+robot green; live re-profile pending `make install`. See `knowledge/bugs/notch-cpu-animation.md` Instance 2.
- [2026-06-29] **"Broke after migrate" ≠ migration — check `~/.claude/settings.json` validity first.** Claude Code **2.1.195** rejects the **entire** settings.json on any single invalid value (`Files with errors are skipped entirely`). An invalid `skillOverrides.deep-research:"disabled"` (valid: `on|name-only|user-invocable-only|off`) silently disabled statusLine, advisorModel, remoteControlAtStartup, tui, model all at once — coinciding with the SwiftUI migration date, so it *looked* like a Harness regression. Fix: `"disabled"`→`"off"`. Diagnostic: `script -q /dev/null claude` (real PTY) surfaces the `SettingsError` startup dialog that background/`-p` sessions hide. `/model` writes settings fine (persist works, only *load* fails). `defaultToAgentsView:true` in `~/.claude.json` sends bare `claude` to the agents dashboard (renders `subagentStatusLine`, not `statusLine`). Remote-control = separate re-auth (`daemon-auth-status.json` auth_required → `claude --remote-control`); advisor on/off is per-session by design. See `knowledge/cases/misc.md` CASE-057.
