...
- RL-052: `Task { }` inside `@MainActor` class inherits MainActor isolation — `Process.waitUntilExit()` blocks main thread → UI freeze. Fix: `Task.detached(priority: .utility)` for all subprocess calls. Triggered by SwiftUI sidebar migration where git/gh commands ran on MainActor.
- RL-053: Calling a method that mutates `@Observable` state from a SwiftUI View `body` creates an infinite re-render loop. `gitMetadata()` mutated `gitMetadataCache` inside a Task → @Observable saw mutation → re-rendered all rows → called `gitMetadata()` again → ∞. Fix: `@ObservationIgnored` on async caches, pass pre-computed values to child views.
