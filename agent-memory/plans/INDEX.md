# Plans Index — harness-terminal

## Active Plans

| File | Title | Status |
|------|-------|--------|
| [p25-ios-ipados-support.md](p25-ios-ipados-support.md) | P25 — iOS/iPadOS Support | Planning |
| [p23-ssh-remote-ui.md](p23-ssh-remote-ui.md) | P23 — SSH Remote Host Manager (PuTTY-style UI) | Active |
| [p8-macos27-adoption.md](p8-macos27-adoption.md) | P8 — macOS 27 Golden Gate Adoption | Active |

## Completed

→ [completed-archive.md](completed-archive.md)

### Quick ref — recent completions

| Plan | Status | Notes |
|------|--------|-------|
| Sidebar SwiftUI Migration (Option B) | Done | NSTableView → SwiftUI List; RL-051 crash class eliminated; VC 1676→890 lines (~47KB reduction); @Observable model + NSHostingView bridge |
| P27 — Pane Drag-and-Drop | Done | Drag grip → drop zone overlay (L/R/T/B/Center) → swapPanes / joinPane with `before` param |
| P26 — Agent Connection | Done | Path A: harness-mcp (existing), Path B: ⌘I inline AI chat (PR #28). ACP remains shelved. |
| P4 — LSP + Code Viewing | Done | `harnessErrors` MCP tool surfaces LSP diagnostics to agents |
| P5 — ACP Client | Shelved | Chat sidebar; re-enable when adapters ship as `brew install` |
| P12 — MCP Server | Done | 27 tools; `harness-mcp`; Claude Code/Codex/Kiro; see `knowledge/architecture/mcp-server.md` |
