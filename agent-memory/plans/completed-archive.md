...
- **Backlog follow-up (2026-07-02, not built) — pane purpose/label tagging for MCP disambiguation.** Surfaced while discussing P34 F3: given a Tab with 2+ panes (e.g. one running Claude Code, one running `swift build`), an MCP agent calling `harnessGetLastBlock(surfaceId:)` has no reliable way to know *which* `surfaceId` is "the build pane" without guessing. `harnessList`'s `agent` field already disambiguates an agent-running pane automatically (`AgentDetector`), but there's no equivalent signal for a plain shell pane. Verified the data model: `title` already lives per-surface on `PaneSurface.title` (`Packages/HarnessIPC/Sources/HarnessIPC/PaneNode.swift:101`, default `"Shell"`), and `IPCRequest.updateTabTitle(surfaceID:title:)` (`IPCMessage.swift:55`) already exists to set it — it's just never exposed as an MCP tool today (only OSC 0/2 from inside the shell drives it currently). Cheapest fix: a thin new MCP tool (e.g. `harnessSetPaneLabel(surfaceId:label:)`) wrapping the existing `updateTabTitle` IPC request — no new schema/field needed, `harnessList`'s existing `title` field would surface it for free. A human or agent could label a pane "build" once, and any orchestrator agent could then find it deterministically via `harnessList` instead of brute-forcing `harnessGetLastBlock` across every candidate pane. Not scoped/estimated — flagged here for a future pass, thematically adjacent to P32's existing `Tab.taskName` metadata pattern.
...
- **Backlog decision (2026-07-02, not yet applied) — reconsider ⌘-click as the block-action trigger.** User pushback after shipping: ⌘-click to select a block and reveal the `BlockActionBar` (Copy/Output/Command/Re-run) is not discoverable — no on-screen hint, has to be memorized, and the user initially assumed ⌘-click meant something else (it doesn't; `zoomPane` is on the Prefix-key combo, not ⌘-click — confirmed no actual conflict, just an assumption from another app). Decision: **don't touch the shipped code yet** — user asked to log this as a direction to revisit rather than redo now. Two directions raised: (a) a more standard/discoverable trigger (e.g. right-click context menu — a pattern users already expect to reveal options), or (b) drop the custom action-bar UI for copy entirely and rely on plain OS-standard ⌘C/⌘V over a manual text selection, accepting the original "manual selection can bleed across block boundaries" tradeoff P34's Why section was trying to solve. Not scoped/estimated.
