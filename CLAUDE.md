...
- **ACP Client is enabled** via `#define HARNESS_ACP` in `Package.swift` (both `HarnessCore` and `HarnessApp` targets). The Agent tab appears as the 4th segment in the sidebar (`["Sessions", "Files", "Git", "Agent"]` at index 3). Adapters are installed via `harness-cli install-acp` which runs `npm install -g @zed-industries/claude-code-acp @zed-industries/codex-acp`. PATH resolution in the .app bundle searches Homebrew + npm-global dirs in addition to system PATH. Tool-call permission gating is the approval bar in `AgentChatPanelView` (existing design).
