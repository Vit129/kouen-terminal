            ...
            LabeledContent("Minimum contrast") {
                HStack(spacing: 8) {
                    Slider(value: Binding(
                        ...
                        set: { model.update(\.minimumContrast, HarnessSettings.clampedContrast($0)) }
                    ), in: 1...21)
                    .frame(width: 180)
...
private struct ColorHexRow: View {
    ...
    init(_ label: String, _ keyPath: WritableKeyPath<HarnessSettings, String?>, fallback: String, model: SettingsModel) {
        self.label = label
