...
final class ThemeService {
    ...
    func setTheme(_ name: String, seedColors: Bool = true) {
        if seedColors {
            ...
            coord.settings.paletteHex = HarnessSettings.normalizedPalette(preset.paletteHex)
    ...
    func applyImportedTheme(_ document: ThemeDocument) {
        ...
        coord.settings.paletteHex = HarnessSettings.normalizedPalette(colors.palette.map { $0.hexString })
        coord.settings.dividerHex = nil
        coord.settings.statusLineHex = nil
        if let appearance = document.appearance {
            if let opacity = appearance.backgroundOpacity {
                coord.settings.backgroundOpacity = HarnessSettings.clampedOpacity(Float(opacity))
            }
            if let blur = appearance.backgroundBlur {
                coord.settings.backgroundBlur = HarnessSettings.clampedBlur(blur)
            }
            if let family = appearance.fontFamily, !family.isEmpty {
                coord.settings.fontFamily = family
            }
            if let size = appearance.fontSize {
                coord.settings.fontSize = HarnessSettings.clampedFontSize(Float(size))
            }
            if let px = appearance.windowPaddingX {
                coord.settings.windowPaddingX = HarnessSettings.clampedPadding(Float(px))
            }
            if let py = appearance.windowPaddingY {
                coord.settings.windowPaddingY = HarnessSettings.clampedPadding(Float(py))
            }
    ...
    func reimportTerminalConfig() {
        if let imported = TerminalConfigImporter.load() {
            coord.settings = HarnessSettings.makeDefaults(imported: imported)
            try? coord.settings.save()
