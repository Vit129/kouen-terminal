...
final class MainWindowController: NSWindowController {
    ...
    static let frameAutosaveName = "KouenMainWindow"
    ...
    /// Frame saved before titlebar zoom; nil when the window is not zoomed by Kouen.
    ...
    private static func resolvedWindowAppearance() -> NSAppearance? {
        ...
        return NSAppearance(named: KouenChrome.current.isDark ? .darkAqua : .aqua)
    }
    ...
    func applyTransparency() {
        ...
        window.backgroundColor = isOpaque
            ? KouenChrome.current.terminalBackground
            : KouenChrome.current.terminalBackground.withAlphaComponent(tintAlpha)
        ...
        let borderColor = settings.windowBorderHex.flatMap { NSColor.fromHex($0) }
            ?? (KouenChrome.current.isDark ? .white : .black)
