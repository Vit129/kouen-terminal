...
/// Layout metrics and chrome helpers; colors come from `KouenChrome.current`.
@MainActor
enum KouenDesign {
    /// The transparent brand mark (`KouenLogo.png`, bundled into the app) for hero tiles in
    ...
    static var chrome: KouenChromePalette { KouenChrome.current }
...
enum KouenMotion {
    ...
    static func animate(
        _ duration: TimeInterval = KouenDesign.Motion.fast,
        timing: CAMediaTimingFunction = KouenDesign.Motion.standardEase,
    ...
    static func crossfade(_ layer: CALayer?, duration: TimeInterval = KouenDesign.Motion.fast) {
        guard let layer else { return }
        let transition = CATransition()
        transition.type = .fade
        transition.duration = duration
        transition.timingFunction = KouenDesign.Motion.standardEase
        layer.add(transition, forKey: "kouenCrossfade")
    }

    /// Gentle infinite halo pulse for "working" agent indicators. Adds/removes a
    /// `transform.scale` + `opacity` animation pair keyed on `"kouenPulse"`.
    static func startPulse(_ layer: CALayer?, minScale: CGFloat = 1.0, maxScale: CGFloat = 1.55, duration: TimeInterval = 1.4) {
        guard let layer else { return }
        if layer.animation(forKey: "kouenPulse") != nil { return }
        ...
        scale.timingFunction = KouenDesign.Motion.standardEase
        ...
        opacity.timingFunction = KouenDesign.Motion.standardEase
        let group = CAAnimationGroup()
        group.animations = [scale, opacity]
        group.duration = duration * 2
        group.repeatCount = .infinity
        layer.add(group, forKey: "kouenPulse")
    }

    static func stopPulse(_ layer: CALayer?) {
        layer?.removeAnimation(forKey: "kouenPulse")
    }
}
...
final class SoftIconButton: NSButton {
    ...
    func applyChrome() {
        KouenDesign.applyIconButtonChrome(to: layer, bounds: bounds, isHovered: isHovered)
...
@MainActor
final class KouenPillButton: NSButton {
    ...
    init(title: String, kind: Kind = .primary) {
        ...
        bezelStyle = .regularSquare
        setButtonType(.momentaryChange)
        layer?.cornerRadius = KouenDesign.Radius.control
        layer?.cornerCurve = .continuous
...
final class ChromeBackdrop: NSView {
    private var role: KouenDesign.ChromeRole
    ...
    init(role: KouenDesign.ChromeRole) {
        self.role = role
    ...
    func update(role: KouenDesign.ChromeRole) {
        self.role = role
        let chrome = KouenDesign.chrome
        let opacity = KouenChrome.backgroundOpacity

        if Self.crossfadeNextUpdate {
            KouenMotion.crossfade(layer, duration: KouenDesign.Motion.fast)
        }
        ...
        // (see KouenChromePalette.build), so a hard divider line is redundant noise.
        hairline.isHidden = true
        hairline.backgroundColor = chrome.border.withAlphaComponent(chrome.isDark ? 0.55 : 0.75).cgColor
        needsLayout = true
    }

    private func material(for role: KouenDesign.ChromeRole) -> NSVisualEffectView.Material {
        switch role {
        case .sidebar:
            return .sidebar
        case .tabBar:
...
/// to a heavier border.
@MainActor
final class KouenOverlayBackground: NSView {
    ...
    init() {
        self.backdrop = KouenOverlayBackground.makeBackdrop()
        super.init(frame: .zero)
        wantsLayer = true
    ...
    private static func makeBackdrop() -> NSView {
        if let glass = RuntimeGlassEffectView.make(cornerRadius: KouenDesign.Radius.overlay) {
            return glass
        }
        let vibrancy = NSVisualEffectView()
...
final class StatusDotView: NSView {
    ...
    func applyStyle() {
        ...
        if working, !NSWorkspace.shared.accessibilityDisplayShouldReduceMotion {
            KouenMotion.startPulse(halo, minScale: 1.0, maxScale: 1.32, duration: 1.6)
        } else {
            KouenMotion.stopPulse(halo)
        }
    }
}
...
final class AgentChipView: NSView {
    ...
    func configure(kind: AgentKind, hex: String) {
        let tint = NSColor.fromHex(hex) ?? KouenDesign.chrome.accent
...
extension BoardColumnKind {
    ...
    var color: NSColor {
        switch self {
        case .needsAttention: return KouenDesign.chrome.waiting
        case .running:        return .systemBlue
        case .done:           return .systemGreen
        case .error:          return KouenDesign.chrome.danger
        case .idle:           return KouenDesign.chrome.idleStatus
        }
    }
}
