...
final class PrefixKeymap {
    ...
    /// The armed prefix, or `nil` when Kouen controls are disabled, or a blanked
    ...
    func rebuildFromSettings() {
        if let raw = SessionCoordinator.shared.settings.effectivePrefixKey,
           let parsed = ParsedShortcut.parse(raw) {
            ...
            // Prefix disabled (Plain/Persistent/Agent without Kouen controls, or blanked key):
    ...
    private func armRepeat() {
        ...
        let ms = KouenOptions.shared.get("repeat-time", scope: .global)?.intValue ?? 500
...
final class PrefixIndicatorWindow {
    ...
    private func makePanel() -> NSWindow {
        ...
        let host = KouenOverlayBackground()
        ...
        label.font = KouenDesign.Typography.kbd
...
final class PrefixCheatsheetWindow {
    ...
    private func build() -> NSWindow {
        ...
        titleRow.spacing = KouenDesign.Spacing.md
        ...
        NSLayoutConstraint.activate([
            titleRow.topAnchor.constraint(equalTo: content.topAnchor, constant: KouenDesign.Spacing.lg + 2),
            titleRow.leadingAnchor.constraint(equalTo: content.leadingAnchor, constant: KouenDesign.Spacing.xl),
            titleRow.trailingAnchor.constraint(equalTo: content.trailingAnchor, constant: -KouenDesign.Spacing.xl),
            stack.topAnchor.constraint(equalTo: titleRow.bottomAnchor, constant: KouenDesign.Spacing.lg),
            stack.leadingAnchor.constraint(equalTo: content.leadingAnchor, constant: KouenDesign.Spacing.xl),
            stack.trailingAnchor.constraint(equalTo: content.trailingAnchor, constant: -KouenDesign.Spacing.xl),
            stack.bottomAnchor.constraint(lessThanOrEqualTo: content.bottomAnchor, constant: -KouenDesign.Spacing.lg),
        ])
    ...
    private func buildGroup(_ group: Group, width: CGFloat) -> NSView {
        ...
        header.font = KouenDesign.Typography.sectionLabel
        ...
        for entry in group.entries {
            ...
            NSLayoutConstraint.activate([
                ...
                action.leadingAnchor.constraint(equalTo: kbdWrap.trailingAnchor, constant: KouenDesign.Spacing.lg),
            ...
            row.widthAnchor.constraint(equalToConstant: width - KouenDesign.Spacing.xl * 2).isActive = true
        }
    ...
    private func pillWrap(_ label: NSTextField) -> NSView {
        ...
        wrap.layer?.cornerRadius = KouenDesign.Radius.badge
