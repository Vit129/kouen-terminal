        ...
        Section("Detection & hooks") {
            ...
            HStack(spacing: 8) {
                Button("Edit agents.json…") {
                    let url = KouenPaths.applicationSupport.appendingPathComponent("agents.json")
                    NSWorkspace.shared.open(url)
                }
                .buttonStyle(.bordered)

                Button("Edit agent-catalog.json…") {
                    let url = KouenPaths.applicationSupport.appendingPathComponent("agent-catalog.json")
    ...
    private func refreshNotifStatus() {
        DesktopNotifier.authorizationStatus { status in
            DispatchQueue.main.async {
                ...
                case .denied:
                    notifStatus = "macOS is blocking notifications for Kouen. Click below to allow them in System Settings ▸ Notifications."
...
private struct AgentRow: View {
    ...
    private func resolveMCPPath() -> String {
        if let dir = Bundle.main.executableURL?.deletingLastPathComponent() {
            let bundled = dir.appendingPathComponent("kouen-mcp")
