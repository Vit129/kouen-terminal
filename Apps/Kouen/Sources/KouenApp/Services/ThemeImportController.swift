...
enum ExternalOpenKind: Equatable {
    /// A `.kouentheme` document — import + install + offer to apply.
    case theme
...
/// Imports `.kouentheme` files opened from the Finder (double-click / "Open With Kouen").
...
enum ThemeImportController {
    ...
    private static func install(_ document: ThemeDocument) {
        _ = try? fileService.install(document, into: KouenPaths.themesDirectory)
    }
    ...
    private static func presentInstallChoice(for document: ThemeDocument) -> InstallChoice {
        ...
        var info = "Add this theme to Kouen."
    ...
    private static func describe(_ error: Error) -> String {
        ...
        case let .unsupportedVersion(version):
            return "This theme uses format version \(version), which this version of Kouen can’t read. Update Kouen and try again."
        case .emptyName:
