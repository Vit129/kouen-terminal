...
struct DefaultTerminalStatus: Equatable {
    ...
    var summary: String {
        if isDefault {
            return "Kouen is default for SSH/Telnet/man-page links, terminal command files, shell scripts, and executables."
        }
...
enum DefaultTerminalManager {
    ...
    private static func defaultHandler(forScheme scheme: String) -> String? {
        guard let url = URL(string: "\(scheme)://kouen.invalid") else { return nil }
        return bundleIdentifierForDefaultApplication(toOpen: url)
    }

    private static func defaultHandlerForCommandFiles() -> String? {
        bundleIdentifierForDefaultApplication(toOpen: URL(fileURLWithPath: "/tmp/kouen-default-terminal.command"))
    }
...
enum DefaultTerminalOpener {
    ...
    /// Finder "New Kouen Window Here" service): it starts a new session instead of a tab. ssh/telnet/
