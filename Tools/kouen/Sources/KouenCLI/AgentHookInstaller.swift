...
/// merge / backup logic lives in `KouenCore.AgentHookInstaller` (shared with the GUI's
...
enum AgentHookInstallerCLI {
    static func run(agentArg: String) {
        ...
        guard AgentHookInstaller.canInstall(kind) else {
            // Known agent, but it has no shell-command hook mechanism — Kouen notifies
            // for it automatically via process detection. Informational, not an error.
            print("\(kind.displayName) is detected automatically — no hook file needed; Kouen notifies when it stops or needs input.")
            exit(0)
        }
        do {
            ...
            if result.needsManualMerge {
                ...
                print("Left it untouched — add the Kouen hook by hand following 'docs/agent-hooks/\(kind.rawValue).md'.")
                exit(0)
            }
            ...
            for legacy in result.removedLegacy {
                print("(removed legacy Kouen hook file at \(legacy.path))")
            }
