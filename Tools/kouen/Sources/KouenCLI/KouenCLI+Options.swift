...
extension KouenCLI {
    static func handleSetOption(_ args: [String], defaultScope: String, client: DaemonClient) throws {
        ...
        // ($KOUEN_SURFACE — tmux: scoped sets apply to the current window); outside
        // a Kouen pane, require -T instead of silently writing a dead option.
        if scope != "global", target == nil {
            target = callingPaneTarget(scope: scope, client: client)
        }
        if scope != "global", target == nil {
            fputs("set-option: \(scope) scope requires -T <target> (or run inside a Kouen pane)\n", kouenStderr)
            exit(1)
        }
    ...
    /// the CLI's "focus" when it runs inside a Kouen pane ($KOUEN_SURFACE).
    ...
    static func callingPaneTarget(scope: String, client: DaemonClient) -> String? {
        guard let surface = ProcessInfo.processInfo.environment["KOUEN_SURFACE"],
