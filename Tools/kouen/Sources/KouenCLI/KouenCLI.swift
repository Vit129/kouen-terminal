...
struct KouenCLI {
        ...
        do {
            ...
            case "daemon":
                guard args.count > 1 else {
                    runDaemonForeground() // execs KouenDaemon; never returns
                }
                switch args[1] {
                case "start":
            ...
            case "agent":
                try Self.handleAgent(Array(args.dropFirst()), client: client)
            case "memo":
    ...
    static func installCLI() throws {
        ...
        let dest = KouenPaths.applicationSupport.appendingPathComponent("bin/kouen-cli")
        ...
        if let daemon = locateDaemonBinary() {
            do {
                let installedDaemon = KouenPaths.applicationSupport.appendingPathComponent("bin/KouenDaemon")
                try copyExecutable(source: daemon, destination: installedDaemon)
                print("daemon: \(installedDaemon.path)")
                let report = try installer.install(daemonPath: installedDaemon, kouenHome: KouenPaths.applicationSupport)
                print("service (\(installer.backendName)): \(report.unitPath.path)")
            } catch {
                fputs("warning: \(installer.backendName) install failed: \(error)\n", kouenStderr)
            }
        } else {
            fputs("warning: KouenDaemon binary not found; service not installed\n", kouenStderr)
        }
    ...
    static func locateDaemonBinary() -> URL? {
        if let override = ProcessInfo.processInfo.environment["KOUEN_DAEMON_PATH"],
           !override.isEmpty, FileManager.default.fileExists(atPath: override) {
            return URL(fileURLWithPath: override)
        }
        let cli = CLIInstallLocator.sourceBinary()
        let candidate = cli.deletingLastPathComponent().appendingPathComponent("KouenDaemon")
        if FileManager.default.fileExists(atPath: candidate.path) { return candidate }
        // The copy `install` placed under the Kouen home.
        let installed = KouenPaths.applicationSupport.appendingPathComponent("bin/KouenDaemon")
        if FileManager.default.fileExists(atPath: installed.path) { return installed }
        // The installed macOS app bundle (no-op on Linux).
        let appCandidate = URL(fileURLWithPath: "/Applications/Kouen.app/Contents/MacOS/KouenDaemon")
