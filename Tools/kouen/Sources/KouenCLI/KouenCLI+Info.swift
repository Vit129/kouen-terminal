...
extension KouenCLI {
    static func printVersion(_ args: [String]) {
        ...
        var report = VersionReport(
            cliVersion: KouenVersion.short,
            cliBuild: KouenVersion.build,
        ...
        if !report.daemonRunning {
            ...
            if build != KouenVersion.build { line += "  [mismatch — restart Kouen.app, or run: kouen-cli install]" }
            print(line)
        } else {
    ...
    static func printThemePreview(_ args: [String]) {
        if args.contains("--all") {
            for (index, theme) in KouenThemeCatalog.allThemes.enumerated() {
                if index > 0 { print("") }
        ...
        let themeName = flagValue(args, flag: "--theme") ?? KouenThemeCatalog.defaultThemeName
        guard let theme = KouenThemeCatalog.theme(named: themeName) else {
    ...
    static func handleInstallShellIntegration(_ args: [String]) {
        ...
        print("Restart your shell (or open a new Kouen pane) to enable prompt marks, the success/failure gutter, and prompt jumping.")
        if failed { exit(1) }
    }
    ...
    static func printUsage() {
        print("""
        kouen-cli — control Kouen terminal sessions
