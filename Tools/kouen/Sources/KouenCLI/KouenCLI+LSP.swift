...
extension KouenCLI {
    ...
    private static func lspConfiguration(path: String, lang: String?) -> LSPServerConfiguration? {
        ...
        if let lang {
            ...
            fileURL = root.appendingPathComponent("KouenLSPProbe.\(extensionForLanguage(lang))")
        } else {
            fileURL = (exists && isDirectory.boolValue) ? inputURL.appendingPathComponent("KouenLSPProbe") : inputURL
        }
