...
/// terminal — the headline `kouen attach` compositor. Unlike `AttachClient`
/// (single-pane passthrough), this lays out every pane with borders, emulates
/// each pane's screen locally with a renderer-free `KouenGridTerminal`, and paints
...
/// `replayScrollback`, feed the bytes into a `KouenGridTerminal`, read its styled
...
public enum WindowAttachClient {
    ...
    public struct Configuration {
        ...
        public var label: String = "kouen-cli attach-window"
        public init() {}
    }

    public static func run(tab selector: TabSelector, configuration: Configuration = Configuration()) throws -> Int32 {
        guard isatty(STDIN_FILENO) != 0, isatty(STDOUT_FILENO) != 0 else {
            fputs("kouen-cli attach-window: stdin/stdout must be a TTY\n", kouenStderr)
            return 64
        }
        if ProcessInfo.processInfo.environment["KOUEN"] != nil {
            // The `$TMUX` analog: this is running inside a Kouen pane. Allowed
            // (handy for testing), but warn so accidental nesting is visible.
            fputs("kouen-cli attach-window: already inside Kouen ($KOUEN set); nesting — detach with the prefix.\n", kouenStderr)
        }
        let client = DaemonClient()
        guard case let .snapshot(snapshot) = try client.request(.getSnapshot) else {
            fputs("kouen-cli attach-window: could not read session snapshot\n", kouenStderr)
            return 1
        }
        guard let tab = resolveTab(snapshot, selector: selector) else {
            fputs("kouen-cli attach-window: no matching tab\n", kouenStderr)
            return 1
        }
        guard let location = locate(tabID: tab.id, in: snapshot) else {
            fputs("kouen-cli attach-window: tab is not in any session\n", kouenStderr)
            return 1
        }
...
private final class WindowSession: @unchecked Sendable {
    ...
    /// All pane work — feeding KouenGridTerminals, compositing, writing stdout —
    /// runs on this serial queue. KouenGridTerminal is not thread-safe, so every
    /// access is funneled here.
    private let renderQueue = DispatchQueue(label: "kouen.window.render")
    private var terminals: [String: KouenGridTerminal] = [:]
    ...
    private func rebuildLayout(initial: Bool) {
        ...
        for rect in rects {
            ...
            if let term = terminals[sid] {
                ...
                guard let term = KouenGridTerminal(cols: rect.cols, rows: rect.rows) else { continue }
                terminals[sid] = term
    ...
    /// pane order and active state. Kouen has no per-pane title, so `pane_title` falls back
    ...
    private func consumeInputCore(_ byte: UInt8, forward: inout Data) {
        ...
        if inPrefix {
            ...
            switch WindowInputRouter.decodeKeySpec(prefixPending) {
            case .incomplete:
                return
            case .literalPrefix:
    ...
    private func feedRoot(_ byte: UInt8, forward: inout Data) {
        ...
        switch WindowInputRouter.decodeKeySpec(rootPending) {
        case .incomplete:
            inRoot = true
        case let .complete(spec):
    ...
    private func focusedCopyGrid() -> KouenGridTerminal? {
    ...
    private func copyModeDecode(_ bytes: [UInt8]) -> WindowInputRouter.KeySpecDecode {
        if bytes.count == 1 {
            switch bytes[0] {
        ...
        return WindowInputRouter.decodeKeySpec(bytes)
    }
