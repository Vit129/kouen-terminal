...
final class GridCompositorTests: XCTestCase {
    ...
    private func snapshot(_ cols: Int, _ rows: Int, _ bytes: String) -> TerminalGridSnapshot {
        guard let term = KouenGridTerminal(cols: cols, rows: rows) else {
            fatalError("KouenGridTerminal create failed")
        }
        term.feed(bytes)
    ...
    func testStatusLineRendered() {
        ...
        let out = comp.render(panes: [pane(0, 0, 80, 23, grid)], status: "kouen")
        XCTAssertTrue(out.contains("kouen"), "status text should appear")
    }
