...
final class LiveResizeTests: XCTestCase {
    ...
    func testBalancedPaddingCentersSubCellRemainder() {
        ...
        let g = KouenTerminalSurfaceView.computeGridGeometry(
    ...
    func testFrozenOriginClampSlidesWhenGrowthCrossesCellBoundary() {
        ...
        let before = KouenTerminalSurfaceView.computeGridGeometry(
        ...
        let after = KouenTerminalSurfaceView.computeGridGeometry(
    ...
    private func drainPreviewHop(_ view: KouenTerminalSurfaceView) {
        view.testingWaitForEmulatorIdle()
    ...
    func testLiveCommitFallsBackToDebounceOnMainConfinedPipeline() {
        ...
        let view = KouenTerminalSurfaceView(offMainParserFramePipeline: false)
    ...
    func testLiveReflowCommitsGridAndPresentsMidDrag() throws {
        ...
        window.isReleasedWhenClosed = false
        defer { window.contentView = nil }
        let view = KouenTerminalSurfaceView(offMainParserFramePipeline: true) // real-time reflow on
        window.contentView = view
        guard view.testingHasRenderer else { throw XCTSkip("renderer unavailable") }
        view.layoutSubtreeIfNeeded()
