...
/// Re-running onboarding from an *older* Kouen.app (Help → Welcome re-opens the wizard) must never
...
final class BinaryInstallerVersionTests: XCTestCase {
    private func makeDir() throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("kouen-binaryinstaller-\(UUID().uuidString)", isDirectory: true)
    ...
    func testBuildNumberProbeParsesCliBuildFromVersionJSON() throws {
        let dir = try makeDir()
        let fake = dir.appendingPathComponent("kouen-cli")
        // A stand-in that emits the same JSON shape as `kouen-cli version --json`.
        try write(#"""
        #!/bin/sh
    ...
    func testBuildNumberProbeReturnsNilForUnsupportedBinary() throws {
        let dir = try makeDir()
        let fake = dir.appendingPathComponent("KouenDaemon")
        try write(#"""
        #!/bin/sh
        exit 1
        """#, to: fake)
