...
final class DaemonBrowserRoutingTests: XCTestCase {
    ...
    /// With two snapshot subscribers — "HarnessGUI" and "compositor" — a browserOpen
    /// request must be forwarded only to "HarnessGUI", not to the compositor.
    func testBrowserRequestRoutedToHarnessGUI() throws {
        ...
        XCTAssertEqual(guiReceived.value, 1, "browserRequest must reach the HarnessGUI subscriber")
