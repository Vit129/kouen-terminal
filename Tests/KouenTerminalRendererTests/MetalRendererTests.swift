...
/// Kouen's own offscreen renderer conformance suite: render a known frame to a texture,
/// read pixels/stats back, and assert structural behavior without an external oracle.
///
/// Set `KOUEN_WRITE_RENDER_SNAPSHOTS=1` to dump PNGs to
/// `$TMPDIR/KouenRenderSnapshots/` for human debugging. CI must rely on the structural
...
final class MetalRendererTests: XCTestCase {
    ...
    func testGlyphAtlasSinglePageEntriesUsePageZero() throws {
        ...
        for scalar in "Kouen".unicodeScalars {
    ...
    func testInlineImageRendersOverCell() throws {
        ...
        var f = FrameBuilder(theme: theme).build(KouenGridTerminal(cols: 2, rows: 2)!.readGrid()!)
    ...
    // MARK: - Kouen-owned renderer guardrails

    func testKouenRendererFixtureDefaultTextReportsPlausibleGlyphStats() throws {
        let (device, renderer) = try makeRenderer()
    ...
    func testKouenRendererFixtureANSIColorBackgroundsFill() throws {
        let (device, renderer) = try makeRenderer()
    ...
    func testKouenRendererFixtureTruecolorGradientFillsEveryCell() throws {
        let (device, renderer) = try makeRenderer()
        let f = frame(
    ...
    func testKouenRendererFixtureSelectionAndSearchHighlightsFill() throws {
        let (device, renderer) = try makeRenderer()
        let grid = snapshot("\u{1b}[?25l    ", cols: 4, rows: 1)
        let selection = KouenTheme.RGBColor(red: 10, green: 20, blue: 30)
        let search = KouenTheme.RGBColor(red: 40, green: 50, blue: 60)
    ...
    func testKouenRendererFixtureUnderlineStylesStrikeAndOverlineEmitDecorations() throws {
        let (device, renderer) = try makeRenderer()
        let f = frame(
    ...
    func testKouenRendererFixtureBlockElementsAreProceduralFills() throws {
        let (device, renderer) = try makeRenderer()
    ...
    func testKouenRendererFixtureBoxDrawingUsesProceduralSpritePath() throws {
        let (device, renderer) = try makeRenderer()
    ...
    func testKouenRendererFixtureCursorStylesPreserveFillGeometry() throws {
        let (device, renderer) = try makeRenderer()
    ...
    func testKouenRendererFixtureLigatureShapingPathReportsPlausibleGlyphs() throws {
        let (device, renderer) = try makeRenderer()
    ...
    private func writeSnapshotIfRequested(texture: MTLTexture, width: Int, height: Int, name: String) {
        guard ProcessInfo.processInfo.environment["KOUEN_WRITE_RENDER_SNAPSHOTS"] == "1" else { return }
        ...
        let dir = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("KouenRenderSnapshots", isDirectory: true)
