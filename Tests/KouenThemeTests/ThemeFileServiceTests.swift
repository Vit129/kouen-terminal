...
final class ThemeFileServiceTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("kouen-theme-tests-\(UUID().uuidString)")
    ...
    private func sampleDocument(name: String = "Dracula") -> ThemeDocument {
        ThemeDocument(definition: KouenThemeCatalog.theme(named: name)!)
    }

    func testExportThenImportRoundTrips() throws {
        let doc = sampleDocument()
        let url = tempDir.appendingPathComponent("dracula.kouentheme")
    ...
    func testInstallUsesSanitizedName() throws {
        ...
        XCTAssertEqual(url.lastPathComponent, "Tokyo Night.kouentheme")
        XCTAssertTrue(FileManager.default.fileExists(atPath: url.path))
    }
    ...
    func testInstalledThemesListsValidFilesOnly() throws {
        ...
        try Data("not a theme".utf8)
            .write(to: tempDir.appendingPathComponent("broken.kouentheme"))
    ...
    func testFileNameSanitization() {
        XCTAssertEqual(ThemeFileService.fileName(for: "Solarized Dark"), "Solarized Dark.kouentheme")
        XCTAssertEqual(ThemeFileService.fileName(for: "a/b:c"), "a-b-c.kouentheme")
        XCTAssertEqual(ThemeFileService.fileName(for: "   "), "theme.kouentheme")
    }

    func testImportMalformedThrows() throws {
        let url = tempDir.appendingPathComponent("bad.kouentheme")
