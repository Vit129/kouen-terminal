...
final class ThemeCatalogEmbedTests: XCTestCase {
    /// `#filePath` is `<repo>/Tests/KouenThemeTests/ThemeCatalogEmbedTests.swift`; three
    /// parent hops reach the repo root.
    private static var repoRoot: URL {
        URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()   // KouenThemeTests
            .deletingLastPathComponent()   // Tests
            .deletingLastPathComponent()   // repo root
    }

    private static var sourceJSON: URL {
        repoRoot.appendingPathComponent("Packages/KouenTheme/Sources/KouenTheme/Resources/themes.json")
    }

    private static var embedSwift: URL {
        repoRoot.appendingPathComponent("Packages/KouenTheme/Sources/KouenTheme/BundledThemesData.swift")
    }
    ...
    private static func embedFileContents(base64: String) -> String {
        "// Generated from Resources/themes.json by `EXPORT_THEMES=1 swift test --filter ThemeCatalogEmbedTests`.\n"
            + "// Do NOT edit by hand. Base64-encoded JSON of [KouenThemeDefinition] (the community theme catalog).\n"
    ...
    func testRegenerateEmbeddedThemes() throws {
        ...
        _ = try JSONDecoder().decode([KouenThemeDefinition].self, from: json)
    ...
    func testEmbeddedThemesMatchSource() throws {
        ...
        let fromSource = try JSONDecoder().decode([KouenThemeDefinition].self, from: sourceData)
        let fromEmbedded = try JSONDecoder().decode([KouenThemeDefinition].self, from: embeddedData)
        XCTAssertEqual(
