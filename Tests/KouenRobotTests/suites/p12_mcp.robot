...
*** Variables ***
${MCP_BIN}    ${EXECDIR}/../../.build/debug/kouen-mcp

*** Test Cases ***
MCP KouenList Returns Sessions
    [Documentation]    P12: read-only kouenList tool returns session data
    ${result}=    Run MCP Request    kouenList    {}
...
MCP ReadPaneOutput Returns Content
    ...
    # Get first surface ID from kouenList
    ${list_result}=    Run MCP Request    kouenList    {}
    Should Contain    ${list_result}    surfaceId

MCP KouenBoard Returns Columns
    [Documentation]    P16: kouenBoard read-only tool returns board state
    ${result}=    Run MCP Request    kouenBoard    {}
    Should Contain    ${result}    columns

MCP Control Denied Without Env Var
    [Documentation]    P12: mutating tools rejected without KOUEN_MCP_ALLOW_CONTROL=1
...
MCP Control Allowed With Env Var
    [Documentation]    P12: mutating tools work with KOUEN_MCP_ALLOW_CONTROL=1
...
Run MCP Request Allowed
    ...
    ${call}=    Set Variable    {"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"${tool_name}","arguments":${arguments}}}
    ${input}=    Catenate    SEPARATOR=\n    ${init}    ${call}
    ${result}=    Run Process    printf    ${input}    |    env    KOUEN_MCP_ALLOW_CONTROL\=1    ${MCP_BIN}
    ...    shell=True    timeout=10s
    RETURN    ${result.stdout}
