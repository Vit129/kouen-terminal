...
*** Variables ***
${REPO_DIR}       /tmp/harness-robot-wt-test
${CLI}            ${CURDIR}/../../.build/debug/harness-cli
...
CLI Isolate Creates Worktree And Session
    [Documentation]    harness-cli new-session --isolate creates a worktree and session
    ${result}=    Run CLI    new-session    --workspace    Default    --isolate
    ...    --branch    robot-feat-1    --repo    ${REPO_DIR}
    Should Be Equal As Integers    ${result.rc}    0
    Directory Should Exist    ${REPO_DIR}/.harness-worktrees
...
Split In Isolated Session Inherits Worktree
    ...
    Should Contain    ${surfaces.stdout}    .harness-worktrees
    App Should Not Crash
