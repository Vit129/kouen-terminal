...
swift build                        # builds harness-cli + harness-mcp
...
# Single suite
robot Tests/HarnessRobotTests/suites/p4_lsp_file_view.robot
robot Tests/HarnessRobotTests/suites/p11_scripting.robot
robot Tests/HarnessRobotTests/suites/p12_mcp.robot
robot Tests/HarnessRobotTests/suites/p13_splits_navigation.robot
robot Tests/HarnessRobotTests/suites/p16_board.robot
...
├── libraries/
│   └── HarnessUILibrary.py    # Python keywords (osascript + CLI wrappers)
├── resources/
│   └── harness.resource       # Shared RF keywords (Split Right, Toggle Sidebar, etc.)
├── suites/
...
- **MCP tests fail** → Ensure `swift build --product harness-mcp` was run
