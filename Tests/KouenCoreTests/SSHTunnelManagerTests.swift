...
final class SSHTunnelManagerTests: XCTestCase {
    ...
    private func host(
        ...
        remoteSocket: String = "/home/rob/.local/share/kouen/kouen.sock",
        sshArgs: [String] = []
    ) -> RemoteHost {
    ...
    func testTunnelSocketPathIsDeterministicAndUnderTunnelsDir() {
        let a = KouenPaths.tunnelSocketURL(forHost: "devbox")
        let b = KouenPaths.tunnelSocketURL(forHost: "devbox")
        ...
        XCTAssertEqual(a.deletingLastPathComponent().standardizedFileURL,
                       KouenPaths.tunnelsDirectory.standardizedFileURL)
    ...
    func testTunnelSocketPathDisambiguatesNamesThatSanitizeAlike() {
        ...
        let dotted = KouenPaths.tunnelSocketURL(forHost: "dev.box")
        let dashed = KouenPaths.tunnelSocketURL(forHost: "dev-box")
    ...
    func testSSHArgumentsCarryHardeningOptionsAndForwardSpec() throws {
        let args = try SSHTunnelManager.sshArguments(
            for: host(remoteSocket: "/run/user/1000/kouen/kouen.sock"),
        ...
        XCTAssertEqual(args.suffix(3), [
            "-L", "/tmp/t.sock:/run/user/1000/kouen/kouen.sock",
            "rob@devbox",
        ])
    }
    ...
    func testControlCharacterInRemoteSocketIsRejected() {
        XCTAssertThrowsError(try SSHTunnelManager.sshArguments(
            for: host(remoteSocket: "/run/kouen\u{0007}/kouen.sock"),
            localSocket: URL(fileURLWithPath: "/tmp/t.sock")))
    }
    ...
    func testEndpointSpawnsTunnelAndBecomesReachable() throws {
        let expectedSocket = KouenPaths.tunnelSocketURL(forHost: "devbox")
    ...
    func testDetectSocketPathTrimsAndReturnsRemoteStdout() throws {
        let path = try SSHTunnelManager.detectSocketPath(
            ...
            makeProcess: { _ in
                let process = Process()
                process.executableURL = URL(fileURLWithPath: "/bin/sh")
                process.arguments = ["-c", "printf '/home/rob/.local/share/kouen/kouen.sock\\n'"]
                return process
            })
        XCTAssertEqual(path, "/home/rob/.local/share/kouen/kouen.sock")
    }
    ...
    func testDetectSocketPathRejectsGarbageOutput() {
        XCTAssertThrowsError(try SSHTunnelManager.detectSocketPath(
            ...
            makeProcess: { _ in
                let process = Process()
                process.executableURL = URL(fileURLWithPath: "/bin/sh")
                process.arguments = ["-c", "printf 'bash: kouen-cli: command not found\\n'"]
                return process
            })) { error in
