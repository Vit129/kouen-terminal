...
final class KouenSettingsTests: XCTestCase {
    private let colorMigrationKey = "KouenColorFidelityMigrationV1"
    ...
    func testSidebarVisibleDefaultsToTrue() {
        XCTAssertTrue(KouenSettings().sidebarVisible)
        XCTAssertTrue(KouenSettings.makeDefaults(imported: nil).sidebarVisible)
    }
    ...
    func testOldSettingsWithCustomHexDoNotSilentlyOverrideThemes() throws {
        ...
        let settings = try JSONDecoder().decode(KouenSettings.self, from: data)
    ...
    func testVividColorsDefaultsToAccurateSRGBWhenMissing() throws {
        XCTAssertFalse(KouenSettings().vividColors)
        XCTAssertEqual(KouenSettings().colorRendering, .accurate)
        XCTAssertFalse(KouenSettings.makeDefaults(imported: nil).vividColors)
        XCTAssertEqual(KouenSettings.makeDefaults(imported: nil).colorRendering, .accurate)

        let legacy = Data("""
        { "fontSize": 14, "customBackgroundHex": "#000000" }
        """.utf8)
        let migrated = try JSONDecoder().decode(KouenSettings.self, from: legacy)
    ...
    func testFreshSettingsUseSlimBarCursorByDefault() {
        XCTAssertEqual(KouenSettings().cursorStyle, "bar")
        XCTAssertEqual(KouenSettings.makeDefaults(imported: nil).cursorStyle, "bar")
    }
    ...
    func testExplicitEventMapWinsOverLegacyCommandFinishedFlag() throws {
        ...
        """.utf8)
        let decoded = try JSONDecoder().decode(KouenSettings.self, from: blob)
    ...
    func testExplicitRenderingModesRoundTripWithLegacyAliases() throws {
        var settings = KouenSettings(colorRendering: .vivid, textRendering: .soft)
        settings.colorGamut = .displayP3

        let encoded = try JSONEncoder().encode(settings)
        let decoded = try JSONDecoder().decode(KouenSettings.self, from: encoded)
    ...
    func testNotchSettingsDefaultAndLegacyDecode() throws {
        XCTAssertEqual(KouenSettings().notchVisibilityMode, .automatic)
        XCTAssertTrue(KouenSettings().notchOpenOnHover)
    ...
    func testNotchSettingsRoundTripAndAutomaticResolution() throws {
        ...
        let decoded = try JSONDecoder().decode(KouenSettings.self, from: try JSONEncoder().encode(settings))
    ...
    func testOffMainParserFramePipelineDefaultsOnAndRoundTrips() throws {
        ...
        XCTAssertTrue(KouenSettings().offMainParserFramePipeline)
        ...
        """.utf8)
        let decodedOptOut = try JSONDecoder().decode(KouenSettings.self, from: optedOut)
    ...
    func testLiveResizeReflowDefaultsOnAndRoundTrips() throws {
        ...
        XCTAssertTrue(KouenSettings().liveResizeReflow)
    ...
    func testRestoreWindowSizeDefaultsOffAndRoundTrips() throws {
        ...
        XCTAssertFalse(KouenSettings().restoreWindowSize)
    ...
    func testImportedDefaultsKeepFullColorSet() {
        ...
        let settings = KouenSettings.makeDefaults(imported: imported)
    ...
    func testClampedOpacityAllowsFullRangeAboveTinyFloor() {
        ...
        XCTAssertEqual(KouenSettings.clampedOpacity(0.01), 0.05, accuracy: 0.001)
        XCTAssertEqual(KouenSettings.clampedOpacity(0.05), 0.05, accuracy: 0.001)
        XCTAssertEqual(KouenSettings.clampedOpacity(0.10), 0.10, accuracy: 0.001)
        XCTAssertEqual(KouenSettings.clampedOpacity(0.30), 0.30, accuracy: 0.001)
        XCTAssertEqual(KouenSettings.clampedOpacity(0.85), 0.85, accuracy: 0.001)
        XCTAssertEqual(KouenSettings.clampedOpacity(1.5), 1.0, accuracy: 0.001)
        XCTAssertEqual(KouenSettings.clampedOpacity(-1.0), 0.05, accuracy: 0.001)
    }

    func testClampedBlurStaysInUsefulRange() {
        XCTAssertEqual(KouenSettings.clampedBlur(-5), 0)
        XCTAssertEqual(KouenSettings.clampedBlur(0), 0)
        XCTAssertEqual(KouenSettings.clampedBlur(20), 20)
        XCTAssertEqual(KouenSettings.clampedBlur(100), 100)
        XCTAssertEqual(KouenSettings.clampedBlur(999), 100)
    }
    ...
    func testClampedFontSizeStaysInZoomRange() {
        ...
        XCTAssertEqual(KouenSettings.clampedFontSize(0), 8, accuracy: 0.001)
        XCTAssertEqual(KouenSettings.clampedFontSize(1), 8, accuracy: 0.001)
        XCTAssertEqual(KouenSettings.clampedFontSize(8), 8, accuracy: 0.001)
        XCTAssertEqual(KouenSettings.clampedFontSize(16), 16, accuracy: 0.001)
        XCTAssertEqual(KouenSettings.clampedFontSize(32), 32, accuracy: 0.001)
        XCTAssertEqual(KouenSettings.clampedFontSize(999), 32, accuracy: 0.001)
        XCTAssertEqual(KouenSettings.clampedFontSize(-5), 8, accuracy: 0.001)
    }

    func testFontSizeIsClampedAtEveryPersistenceBoundary() throws {
        // init clamps.
        XCTAssertEqual(KouenSettings(fontSize: 999).fontSize, 32, accuracy: 0.001)
        XCTAssertEqual(KouenSettings(fontSize: 1).fontSize, 8, accuracy: 0.001)

        // init(from:) clamps a decoded out-of-range value.
        let huge = try JSONDecoder().decode(KouenSettings.self, from: Data(#"{ "fontSize": 999 }"#.utf8))
        XCTAssertEqual(huge.fontSize, 32, accuracy: 0.001)
        let tiny = try JSONDecoder().decode(KouenSettings.self, from: Data(#"{ "fontSize": 1 }"#.utf8))
        XCTAssertEqual(tiny.fontSize, 8, accuracy: 0.001)
        let negative = try JSONDecoder().decode(KouenSettings.self, from: Data(#"{ "fontSize": -5 }"#.utf8))
    ...
    func testLoadClampsRunawayFontSizeAndPaddingAndRewritesOnce() throws {
        try withTemporaryKouenHome { root in
            ...
            let reloaded = try JSONDecoder().decode(KouenSettings.self, from: try Data(contentsOf: url))
    ...
    func testClampedPaddingNeverNegative() {
        XCTAssertEqual(KouenSettings.clampedPadding(-10), 0, accuracy: 0.001)
        XCTAssertEqual(KouenSettings.clampedPadding(0), 0, accuracy: 0.001)
        XCTAssertEqual(KouenSettings.clampedPadding(14), 14, accuracy: 0.001)
        XCTAssertEqual(KouenSettings(windowPaddingX: -5, windowPaddingY: -1).windowPaddingX, 0, accuracy: 0.001)
    }
    ...
    func testCorruptSettingsAreBackedUpNotOverwritten() throws {
        try withTemporaryKouenHome { root in
            ...
            let settings = KouenSettings.load()
            XCTAssertEqual(settings.fontSize, KouenSettings.makeDefaults(imported: nil).fontSize)
    ...
    func testFontSizeIsKouenOwnedNotImported() {
        // A source terminal's font *size* must not carry over (only the face does), so the
        // Kouen default size wins even when the imported config specifies one.
        let imported = ImportedTerminalConfig(
            fontFamily: "JetBrainsMono Nerd Font",
            fontSize: 17,
        ...
        XCTAssertEqual(settings.fontSize, KouenSettings().fontSize)  // size is the Kouen default
    ...
    private func withTemporaryKouenHome(_ body: (URL) throws -> Void) throws {
        ...
        let root = URL(fileURLWithPath: "/tmp/kouen-settings-\(UUID().uuidString.prefix(8))", isDirectory: true)
