...
final class FormatStringTests: XCTestCase {
    ...
    func testSubstitutesSimpleTokens() {
        ...
        XCTAssertEqual(result, "[work] kouen")
    }
    ...
    func testStatusRightDefaultOmitsBranchSegmentWhenMissing() {
        ...
        let formatter = DateFormatter()
        formatter.dateFormat = "HH:mm"
        let expected = " kouen · \(formatter.string(from: ctx.now)) "
...
final class OptionStoreTests: XCTestCase {
    ...
    /// tmux's `default-terminal` maps onto Kouen's terminal-identity option — one
    /// storage slot, both names (P4 alias), for set, get, AND unset.
    func testDefaultTerminalAliasesTerminalIdentity() {
        let store = OptionStore(url: tmpURL())
        store.set(.string("kouen"), key: "default-terminal")
        XCTAssertEqual(store.get(TerminalIdentity.optionKey)?.stringValue, "kouen")
        XCTAssertEqual(store.get("default-terminal")?.stringValue, "kouen")
...
final class FormatStringExtendedVariableTests: XCTestCase {
    ...
    func testClientAndServerTokens() {
        ...
        XCTAssertEqual(FormatString.evaluate("#{version}", context: context), KouenVersion.short)
