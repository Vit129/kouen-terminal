...
/// Coverage for the CLI's pure argument-parsing helpers. `kouen-cli` previously had no test
...
final class KouenCLITests: XCTestCase {
    func testFlagValueReturnsTheFollowingToken() {
        XCTAssertEqual(KouenCLI.flagValue(["--tab", "abc"], flag: "--tab"), "abc")
        XCTAssertEqual(KouenCLI.flagValue(["--cwd", "~", "--tab", "id"], flag: "--tab"), "id")
    }
    ...
    func testFlagValueIsNilWhenFlagHasNoValue() {
        ...
        XCTAssertNil(KouenCLI.flagValue(["close-tab", "--tab"], flag: "--tab"))
    }

    func testFlagValueIsNilWhenFlagAbsent() {
        XCTAssertNil(KouenCLI.flagValue(["--workspace", "Default"], flag: "--tab"))
        XCTAssertNil(KouenCLI.flagValue([], flag: "--tab"))
    }

    func testFlagValueTakesFirstOccurrence() {
        XCTAssertEqual(KouenCLI.flagValue(["--tab", "first", "--tab", "second"], flag: "--tab"), "first")
    }
    ...
    func testFlagValueTakesNextTokenVerbatimEvenIfFlagLike() {
        ...
        XCTAssertEqual(KouenCLI.flagValue(["--tab", "--oops"], flag: "--tab"), "--oops")
    }
    ...
    func testKouenViewLoadsPlainUTF8() throws {
        let dir = try makeTemporaryDirectory()
        let file = dir.appendingPathComponent("sample.txt")
        try "hello\nworld\n".write(to: file, atomically: true, encoding: .utf8)
        XCTAssertEqual(try KouenFilePreviewLoader.load(path: file.path), "hello\nworld\n")
    }

    func testKouenViewRejectsOversizedFiles() throws {
        let dir = try makeTemporaryDirectory()
        let file = dir.appendingPathComponent("large.txt")
        try Data(repeating: 0x61, count: KouenFilePreviewLoader.maxPreviewBytes + 1).write(to: file)
        XCTAssertThrowsError(try KouenFilePreviewLoader.load(path: file.path)) { error in
            XCTAssertEqual(error as? KouenViewError, .tooLarge(KouenFilePreviewLoader.maxPreviewBytes + 1))
        }
    }

    func testKouenViewRejectsBinaryFiles() throws {
        let dir = try makeTemporaryDirectory()
        let file = dir.appendingPathComponent("binary.bin")
        try Data([0xff, 0xfe, 0x00]).write(to: file)
        XCTAssertThrowsError(try KouenFilePreviewLoader.load(path: file.path)) { error in
            XCTAssertEqual(error as? KouenViewError, .binaryOrUnsupportedEncoding)
        }
    }

    private func makeTemporaryDirectory() throws -> URL {
        let url = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("KouenCLITests-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: url, withIntermediateDirectories: true)
        return url
    }

    /// The hand-maintained mirror of the `KouenCLI.main` dispatch `switch`: the canonical verb of
    /// each `case` (the first label, excluding pure aliases like `--version`/`setw`). Whenever a
    /// `case` is added to or removed from the dispatch in `KouenCLI.swift`, update this set; the
    ...
    func testEveryCatalogCommandIsDispatched() {
        ...
        XCTAssertTrue(phantom.isEmpty,
                      ...
                      + "Add a dispatch case in KouenCLI.swift (and to dispatchVerbs), or allowlist it in nonDispatchCatalogVerbs with a reason.")
    }
    ...
    func testBindHookRejectsLeadingIfFlagWithoutTrapping() {
        ...
        XCTAssertNil(KouenCLI.parseBindHook(["--if", "cond"]))
    }
    ...
    func testBindHookRejectsIfFlagImmediatelyAfterEvent() {
        ...
        XCTAssertNil(KouenCLI.parseBindHook(["ev", "--if", "fmt"]))
    }

    func testBindHookRejectsDanglingIfFlag() {
        // `--if` with no format token following it is malformed.
        XCTAssertNil(KouenCLI.parseBindHook(["ev", "cmd", "--if"]))
    }

    func testBindHookRejectsTooFewTokens() {
        XCTAssertNil(KouenCLI.parseBindHook([]))
        XCTAssertNil(KouenCLI.parseBindHook(["ev"]))
    }

    func testBindHookParsesWellFormedWithCondition() {
        let parsed = KouenCLI.parseBindHook(["ev", "cmd", "--if", "fmt"])
    ...
    func testBindHookParsesMultiTokenCommandWithCondition() {
        let parsed = KouenCLI.parseBindHook(["ev", "new-window", "-h", "--if", "fmt"])
        XCTAssertEqual(parsed?.event, "ev")
    ...
    func testBindHookParsesWithoutCondition() {
        let parsed = KouenCLI.parseBindHook(["ev", "new-window", "-h"])
        XCTAssertEqual(parsed?.event, "ev")
        XCTAssertEqual(parsed?.source, "new-window -h")
    ...
    func testParseDetachSequenceRejectsPlausibleBadInputs() {
        ...
        XCTAssertNil(KouenCLI.parseDetachSequence("ctrl-d"))
        XCTAssertNil(KouenCLI.parseDetachSequence("^A,d"))
        XCTAssertNil(KouenCLI.parseDetachSequence("300"))   // out of UInt8 range
        XCTAssertNil(KouenCLI.parseDetachSequence(""))
    }

    func testParseDetachSequenceAcceptsValidFormats() {
        XCTAssertEqual(KouenCLI.parseDetachSequence("C-a d"), [0x01, 0x64])
        XCTAssertEqual(KouenCLI.parseDetachSequence("0x01 0x64"), [0x01, 0x64])
        XCTAssertEqual(KouenCLI.parseDetachSequence("1,100"), [1, 100])
    }
    ...
    func testResolveDetachSequenceAbsentFlagKeepsDefault() {
        ...
        XCTAssertEqual(KouenCLI.resolveDetachSequence(["attach", "--surface", "x"]), .absent)
    }

    func testResolveDetachSequenceValidFlagParses() {
        XCTAssertEqual(
            KouenCLI.resolveDetachSequence(["--detach-keys", "C-a d"]),
            .parsed([0x01, 0x64]))
    }
    ...
    func testResolveDetachSequenceInvalidFlagFailsLoudly() {
        ...
        guard case .invalid(let message) =
            KouenCLI.resolveDetachSequence(["--detach-keys", "ctrl-d"]) else {
    ...
    func testResolveDetachSequenceDanglingFlagFailsLoudly() {
        ...
        guard case .invalid(let message) =
            KouenCLI.resolveDetachSequence(["attach", "--surface", "x", "--detach-keys"]) else {
    ...
    func testOptionalUUIDFlagAbsentIsAbsent() {
        ...
        XCTAssertEqual(
            KouenCLI.optionalUUIDFlag(["new-split", "--tab", "t"], flag: "--pane"),
            .absent)
    }
    ...
    func testOptionalUUIDFlagValidRoundTrips() {
        let uuid = UUID()
        XCTAssertEqual(
            KouenCLI.optionalUUIDFlag(["--pane", uuid.uuidString], flag: "--pane"),
            .valid(uuid))
    }
    ...
    func testOptionalUUIDFlagInvalidFailsWithRawValue() {
        ...
        XCTAssertEqual(
            KouenCLI.optionalUUIDFlag(["--pane", "not-a-uuid"], flag: "--pane"),
            .invalid("not-a-uuid"))
    }
    ...
    func testOptionalUUIDFlagDanglingIsDistinctFromAbsent() {
        ...
        XCTAssertEqual(
            KouenCLI.optionalUUIDFlag(["new-split", "--tab", "X", "--pane"], flag: "--pane"),
            .dangling)
    }

    func testFlagIsDanglingDistinguishesAbsentDanglingAndValued() {
        // Present with a following value → not dangling.
        XCTAssertFalse(KouenCLI.flagIsDangling(["--pane", "v"], flag: "--pane"))
        // Present as the last token → dangling.
        XCTAssertTrue(KouenCLI.flagIsDangling(["new-split", "--pane"], flag: "--pane"))
        // Absent entirely → not dangling (it's just absent).
        XCTAssertFalse(KouenCLI.flagIsDangling(["--tab", "t"], flag: "--pane"))
    }
    ...
    func testParseKeyTableArgsDefaultsToPrefixWithoutEatingPositionals() {
        ...
        let (table, positional) = KouenCLI.parseKeyTableArgs(["bind-key", "prefix", "new-window"])
    ...
    func testParseKeyTableArgsStripsExplicitTableValue() {
        ...
        let (table, positional) = KouenCLI.parseKeyTableArgs(["bind-key", "-T", "root", "C-b", "detach-client"])
    ...
    func testParseKeyTableArgsExplicitPrefixTableStripsOnlyTheTableToken() {
        ...
        let (table, positional) = KouenCLI.parseKeyTableArgs(["bind-key", "-T", "prefix", "prefix", "new-window"])
        XCTAssertEqual(table, "prefix")
        XCTAssertEqual(positional, ["prefix", "new-window"])
    }
