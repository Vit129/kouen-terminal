...
/// P16 PBI-BOARD-003: `kouen board` renders `BoardModel.classify(...)` as a
...
final class BoardCommandTests: XCTestCase {
    func testRenderBoardShowsAllColumnsWithCounts() {
        ...
        let output = KouenCLI.renderBoard(BoardModel.classify(snapshot: snap))
    ...
    func testRenderBoardShowsPlaceholderForEmptyColumns() {
        let output = KouenCLI.renderBoard(BoardModel.classify(snapshot: SessionSnapshot()))
