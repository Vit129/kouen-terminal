...
final class EngineConformanceTests: XCTestCase {
    ...
    // MARK: - Kouen-owned conformance fixtures

    /// These fixtures are Kouen's own conformance suite: small, inline examples that lock
    /// protocol behavior without importing any external terminal test corpus.

    func testKouenFixtureSGRBasicColors() {
    ...
    func testKouenFixtureSGRTruecolorForegroundAndBackground() {
    ...
    func testKouenFixtureTextStyleSGRAttributes() {
    ...
    func testKouenFixtureStrikeOverlineAndResets() {
    ...
    func testKouenFixtureCursorMovementCUPAndCHA() {
    ...
    func testKouenFixtureEraseInLineModeOneClearsLeftOfCursor() {
    ...
    func testKouenFixtureEraseInDisplayModeZeroClearsFromCursorForward() {
    ...
    func testKouenFixtureInsertAndDeleteCharactersPreserveAttributes() {
    ...
    func testKouenFixtureInsertAndDeleteLinesStayInsideScrollRegion() {
    ...
    func testKouenFixtureScrollRegionKeepsOutsideRowsPinned() {
    ...
    func testKouenFixtureAlternateScreenRestoresPrimaryTextAndAttributes() {
        let term = KouenGridTerminal(cols: 8, rows: 2)!
        term.feed("\u{1b}[31mP")
    ...
    func testKouenFixtureAutowrapCanBeDisabledAndReenabled() {
    ...
    func testKouenFixtureBracketedPasteModeDrivesInputEncoder() {
        let term = KouenGridTerminal(cols: 8, rows: 1)!
    ...
    func testKouenFixtureOSC8HyperlinksUseStableCellIDs() {
        let term = KouenGridTerminal(cols: 8, rows: 1)!
    ...
    func testKouenFixtureOSC52DecodesUnicodeClipboardPayload() {
        let term = KouenGridTerminal(cols: 8, rows: 1)!
        var captured: String?
    ...
    func testKouenFixtureOSC133MarksCarryExitStatusInSnapshot() {
        let term = KouenGridTerminal(cols: 12, rows: 2)!
    ...
    func testKouenFixtureDECSpecialGraphicsMapsLineDrawing() {
    ...
    func testKouenFixtureWideCharacterMarksTailCell() {
    ...
    func testKouenFixtureCombiningMarkStaysOnBaseCell() {
    ...
    func testKouenFixtureDECSCUSRUnderlineCursorShape() {
    ...
    func testDECSCUSRCursorShape() {
        let term = KouenGridTerminal(cols: 10, rows: 2)!
        ...
        XCTAssertEqual(KouenGridTerminal(cols: 4, rows: 1)!.readGrid()!.cursor.shape, .default)
    }
    ...
    func testDECSCUSRReplayedTailEndsOnDefault() {
        ...
        let term = KouenGridTerminal(cols: 20, rows: 2)!
        term.feed(tail)
        XCTAssertEqual(term.readGrid()!.cursor.shape, .default)
    ...
    func testDetectFilePathStripsToolCallParens() {
        ...
        // before the path, e.g. "⏺ Update(Apps/Kouen/.../SyntaxTextView.swift)".
        let line = "⏺ Update(Apps/Kouen/Sources/KouenApp/UI/FileEditor/SyntaxTextView.swift)"
    ...
    func testSynchronizedOutputModeAndDECRQM() {
        let term = KouenGridTerminal(cols: 20, rows: 4)!
        var responses = Data()
        term.onResponse = { responses.append($0) }
    ...
    func testXTVersionResponse() {
        ...
        let emu = TerminalEmulator(cols: 80, rows: 24)
        emu.terminalName = "Kouen"
        emu.terminalVersion = "1.3.0"
        var response = Data()
        emu.onResponse = { response.append($0) }
        emu.feed("\u{1b}[>q")
        XCTAssertEqual(String(data: response, encoding: .utf8), "\u{1b}P>|Kouen 1.3.0\u{1b}\\")
    }
    ...
    func testXTVersionUsesCompatibleIdentityWhenSet() {
        ...
        emu.onResponse = { response.append($0) }
        emu.feed("\u{1b}[>q")
        XCTAssertEqual(String(data: response, encoding: .utf8), "\u{1b}P>|ghostty 1.3.0\u{1b}\\")
    }
    ...
    func testScrollRegionConstrainsLineFeed() {
        ...
        let term = KouenGridTerminal(cols: 10, rows: 4)!
    ...
    func testBareDECSTBMHomesOnSingleRowGrid() {
        ...
        let term = KouenGridTerminal(cols: 10, rows: 1)!
