...
/// Performance baselines for Kouen's hot paths. These exist to catch regressions, not to
/// gate CI — `measure {}` runs each body ~10×, which is too slow for the default suite, so the
/// whole file is opt-in. Run with:
///
///     KOUEN_BENCHMARKS=1 swift test --filter KouenBenchmarks
///
...
/// --filter KouenBenchmarks` (still gated on KOUEN_BENCHMARKS=1).
final class PerformanceBenchmarks: XCTestCase {
    ...
    // (`KouenTerminalSurfaceView.receive` → emulator worker hop → main hop → scheduler) with a
    ...
    /// build log / chatty TUI produces, and the workloads Kouen trails Ghostty on.
    ...
    func testReadGridSnapshotFullScreen() throws {
        try skipUnlessEnabled()
        let term = KouenGridTerminal(cols: 200, rows: 60)!
    ...
    func testScrollTickShiftVsFullRebuild() throws {
        ...
        let theme = KouenThemeCatalog.theme(named: "Dracula")!
        let builder = FrameBuilder(theme: theme)
        let ticks = 200
    ...
    func testOutputScrollHintBuildCost() throws {
        ...
        let fullTerm = KouenGridTerminal(cols: cols, rows: rows)!
        ...
        let hintTerm = KouenGridTerminal(cols: cols, rows: rows)!
    ...
    func testCompositorFrameBuildFourPanes() throws {
        ...
        func snapshot(_ cols: Int, _ rows: Int) -> TerminalGridSnapshot {
            let t = KouenGridTerminal(cols: cols, rows: rows)!
        ...
        let nanos = timedNanos {
            for i in 0 ..< 40 {
                comp.invalidate()
                _ = comp.render(panes: panes, status: "kouen · bench \(i)")
            }
        }
