...
/// Uses an isolated `KOUEN_HOME` so `ensureDirectories` / `scrollbackFileURL` resolve into a
...
final class ScrollbackFileTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        home = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("kouen-scrollback-\(UUID().uuidString)", isDirectory: true)
    ...
    private func url(_ id: String = UUID().uuidString) -> URL {
        KouenPaths.scrollbackFileURL(forSurfaceID: id)
    }
