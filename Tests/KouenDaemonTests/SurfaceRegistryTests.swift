...
/// isolated `KOUEN_HOME` temp dir so it never touches real session state. Creating a
...
final class SurfaceRegistryTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("kouen-registry-\(UUID().uuidString)", isDirectory: true)
    ...
    func testInvalidConfiguredSplitShellFallsBackToExecutableShell() throws {
        let registry = SurfaceRegistry()
        let invalidShell = "/tmp/kouen-missing-split-shell-\(UUID().uuidString)"
        let tab = try XCTUnwrap(registry.snapshot.activeWorkspace?.activeTab)
        let paneID = try XCTUnwrap(tab.rootPane.allPaneIDs().first)
    ...
    func testInvalidConfiguredShellFallsBackToExecutableShell() throws {
        let registry = SurfaceRegistry()
        let invalidShell = "/tmp/kouen-missing-shell-\(UUID().uuidString)"
        let wsID = registry.snapshot.activeWorkspaceID!
    ...
    private func saveSettings(defaultShell: String) throws {
        var settings = KouenSettings(defaultShell: defaultShell)
    ...
    func testScrollbackSweepKeepsReferencedDeletesOrphan() throws {
        ...
        let referencedFile = KouenPaths.scrollbackFileURL(forSurfaceID: referencedSurface.uuidString)
        let orphanFile = KouenPaths.scrollbackFileURL(forSurfaceID: UUID().uuidString)
