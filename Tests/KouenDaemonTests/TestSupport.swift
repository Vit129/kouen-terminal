...
/// here and run with `KOUEN_LIVE_DAEMON_TESTS=1 swift test`. The deterministic logic
/// (IPC codec, paths, SessionEditor) is always covered in KouenCoreTests.
/// The daemon *executable* ignores SIGPIPE (`KouenDaemonMain`), but the XCTest process running
...
func skipUnlessLiveDaemonTests() throws {
    ...
    try XCTSkipUnless(
        ProcessInfo.processInfo.environment["KOUEN_LIVE_DAEMON_TESTS"] == "1",
        "Set KOUEN_LIVE_DAEMON_TESTS=1 to run live daemon/PTY integration tests."
    )
}
