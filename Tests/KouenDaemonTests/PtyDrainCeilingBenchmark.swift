...
/// risky non-blocking-master rework) is worth doing. Gated by both `KOUEN_BENCHMARKS=1` and
/// `KOUEN_LIVE_DAEMON_TESTS=1` (it forks a child), so the normal suites never run it.
final class PtyDrainCeilingBenchmark: XCTestCase {
    private func skipUnlessEnabled() throws {
        _ = testSIGPIPEIgnored
        let env = ProcessInfo.processInfo.environment
        try XCTSkipUnless(
            env["KOUEN_BENCHMARKS"] == "1" && env["KOUEN_LIVE_DAEMON_TESTS"] == "1",
            "Set KOUEN_BENCHMARKS=1 and KOUEN_LIVE_DAEMON_TESTS=1 to run the PTY drain ceiling benchmark."
        )
    }
