...
/// End-to-end IPC: a real `DaemonServer` on a temp-`KOUEN_HOME` socket, driven by a
...
final class DaemonRoundTripTests: XCTestCase {
    ...
    func testControlSocketIsOwnerOnly() throws {
        ...
        let attrs = try FileManager.default.attributesOfItem(atPath: KouenPaths.socketURL.path)
    ...
    func testInputFrameOnSubscriptionConnectionReachesPTY() throws {
        ...
        let marker = "KOUEN_INPUT_FRAME_OK"
    ...
    /// `kouen-cli attach` never call identifyClient). Regression for the off-queue
    ...
    private func connectRawSocket() throws -> Int32 {
        ...
        let path = KouenPaths.socketURL.path
