...
final class WorktreeIsolationDaemonTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("kouen-wt-daemon-\(UUID().uuidString)", isDirectory: true)
    ...
    func testCloseSessionRunsArchiveScriptBeforeRemoval() throws {
        ...
        // A kouen.json with archiveScript that writes a marker file OUTSIDE the worktree
        ...
        """
        try config.write(toFile: wtPath + "/kouen.json", atomically: true, encoding: .utf8)
        shell("git add kouen.json && git commit -m cfg", in: wtPath)
